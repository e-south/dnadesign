...
| Status aggregation | The cheap stress_ethanol_cipro_growth status snapshot exposes `sequence_view_contract_state` and `infer_feature_completion_state` for product contracts, generated sidecar freshness, and nonblocking feature-completion awareness. | `src/dnadesign/studies/units/stress_ethanol_cipro_growth/status/service.py`, `src/dnadesign/studies/units/stress_ethanol_cipro_growth/status/snapshot.py` |
...
| Ops/status sequence-view and Infer completion summaries | `uv run pytest -q src/dnadesign/studies/units/stress_ethanol_cipro_growth/tests/status/test_snapshot.py::test_build_stress_ethanol_cipro_growth_status_surfaces_sequence_view_and_feature_completion_sections` |
