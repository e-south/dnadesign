...
| --- | --- | --- |
| GenBank catalog | Study-owned plasmid and subcomponent sequence authority for RT-lnRNA variants. The catalog currently has 42 representable variants: 41 whole-plasmid sources and one BL21 lnRNA-only source paired with Eco1 WT RT. | All 42 are first-class Construct subjects when their parsed lnRNA and RT CDS authority passes the projection manifest. |
...
The current consolidated source universe is 10,421 Construct subjects:
42 GenBank, 4,148 Crawford, 71 Khan, 80 MSD compiler, and 6,080 RT-CDS DMS.
Construct emits 20,842 realized context rows and 62,526 sequence-view rows for
that universe.
