...
**Entry artifact:** `docs/studies/stress_ethanol_cipro_growth/` plus the study-owned execution surfaces declared in `ops.study.yaml`
...
Use this after [status](status-contract.md) when the question is blocker or
...
`ops.study.yaml`. It does not submit jobs, mutate USR datasets, advance Notify
...
Use [routes](routes.md) for the owner handoff after blockers are clear.
