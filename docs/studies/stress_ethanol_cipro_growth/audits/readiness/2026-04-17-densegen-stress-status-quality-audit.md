## DenseGen + Stress Study Status Quality Audit
...
- stress_ethanol_cipro_growth status and study-record organization
- cross-tool boundary quality where stress_ethanol_cipro_growth status surfaces reach into DenseGen
...
#### 1. Cross-tool boundary violation in stress_ethanol_cipro_growth status
...
- `src/dnadesign/studies/units/stress_ethanol_cipro_growth/operations/status/analysis_surfaces.py:294-299`
- `src/dnadesign/densegen/__init__.py:14-29`
- `src/dnadesign/densegen/tests/config/test_public_api_module_layout.py:20-34`

#### 2. Silent fallback and duplicated DenseGen knowledge inside stress study status

Severity: high

The stress_ethanol_cipro_growth snapshot swallows DenseGen import failures and
substitutes empty plot specs. It also hardcodes DenseGen default and optional
plot ids in study status package code.
...
- Tool-local knowledge is duplicated in the study status package instead of
...
- `DESIGN.md:20-23`
- `src/dnadesign/studies/units/stress_ethanol_cipro_growth/operations/status/analysis_surfaces.py:83-85`
- `src/dnadesign/studies/units/stress_ethanol_cipro_growth/operations/status/analysis_surfaces.py:294-312`
...
#### 5. Stress study status overstates DenseGen surface health
...
- `src/dnadesign/studies/units/stress_ethanol_cipro_growth/operations/status/analysis_surfaces.py:87-92`
- `src/dnadesign/studies/units/stress_ethanol_cipro_growth/operations/status/analysis_surfaces.py:95-120`
...
`routes/README.md` does not declare those fields. `record/status.md` also leaks tool-local
...
- `src/dnadesign/studies/units/stress_ethanol_cipro_growth/operations/status/analysis_surfaces.py:56-120`

### Pragmatic-Principles Readout

Skill composition: paired `deep-introspection` + `stress-ethanol-cipro-growth-status` +
...
- DRY knowledge: DenseGen plot taxonomy is duplicated across workspace config,
  pipeline record, notebook inventory, and study status package code.
...
- No silent fallback: study status package analysis must report
...
5. Tighten study-doc routing.
   - `record/status.md` stays factual and short.
   - `routes/README.md` gets explicit metadata headers and only one-hop routing.
...
- `uv run pytest -q src/dnadesign/studies/units/stress_ethanol_cipro_growth/tests/operations/status/test_snapshot.py src/dnadesign/studies/units/stress_ethanol_cipro_growth/tests/operations/status/test_latentdna.py src/dnadesign/densegen/tests/config/test_public_api_module_layout.py src/dnadesign/usr/tests/test_usr_docs_contract.py`
