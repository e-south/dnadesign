...
The stress study `record/status.md` and `routes/README.md` present `appendix_umap_gallery` as part of the current attention surface, but the LatentDNA notebook only exposes that preset when all eight canonical views are materialized, and its actual notebook surface is `latent_geometry_browser`.
...
- Study pipeline default plot claim: `docs/studies/stress_ethanol_cipro_growth/operations/pipeline.yaml`
