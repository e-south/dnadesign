...
- `ops.study.yaml`: lifecycle, current phase, artifacts, execution surfaces, and preflight shape
- `datasets.yaml`: affiliated dataset ids, row counts, and sync posture
- `pipeline.yaml`: DenseGen, Infer, LatentDNA, Cluster, and OPAL bindings for this study
- `routes.md`: one-hop handoffs into owner tools
- `status.md`: human-readable current-state note
...
[routes](routes.md) after the state question is answered and the next task is
