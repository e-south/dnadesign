...
| Interactive browser structure review | `../../../../src/dnadesign/thread/structure_views/` and `../../../../src/dnadesign/studies/units/eco1_rt_repack/workspaces/eco1_rt_conservative_v1/outputs/thread/review_deliverables/structure_browser/` | materialized interactive inspection for the Ec86 reference and fitted fold models; uses py3Dmol through the generic `dnadesign.thread.structure_views` contract. The constraint-evidence section highlights one selected mask-evidence category at a time on the ec86kit/7V9U reference using a single high-contrast highlight color. The design/fold triage section reuses baseline foldcheck_review PDBs for audit. The panel-selection section uses the expanded fold-check structure set and shows WT plus the selected six with pLDDT, RMSD, mutation count, MSA support, nucleic-acid-facing mutation count, charge change, and chemistry warnings beside the viewer. Structure group labels are section-specific, and molecule-visibility controls stay stable while moving between visuals. Raw local PDB files remain unchanged, and the browser view does not replace ColabFold fold validation |
