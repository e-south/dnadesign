...
| --- | --- | --- |
| Generic fixed-backbone IA and candidate-review dev spec | `../../../dev/plans/cross-tool/thread/2026-06-19-eco1-rt-repack-thread.md` | active next-slice spec for assay-panel preparation: feasibility, three-window SAE summary, candidate triage, six-slot selection, and RT-only handoff |
...
| Review deliverables foundation | `../../../../src/dnadesign/studies/units/eco1_rt_repack/operations/materialization/review_deliverables/` and `../../../../src/dnadesign/studies/units/eco1_rt_repack/workspaces/eco1_rt_conservative_v1/outputs/thread/review_deliverables/` | materialized review bundle; writes mask/MSA evidence, baseline ProteinMPNN audit plots, fold-review links, reference and fold structure browsers, selected-panel structure browsing, expanded-pool panel-selection visuals, and a marimo notebook. ESMC LLR, 300M-versus-6B score comparison, SAE feature review, and WT model-check plots remain supporting context rather than panel-selection evidence. |
| Interactive browser structure review | `../../../../src/dnadesign/thread/structure_views/` and `../../../../src/dnadesign/studies/units/eco1_rt_repack/workspaces/eco1_rt_conservative_v1/outputs/thread/review_deliverables/structure_browser/` | materialized interactive inspection for the Ec86 reference and fitted fold models; uses py3Dmol through the generic `dnadesign.thread.structure_views` contract. The constraint-evidence section highlights one selected mask-evidence category at a time on the ec86kit/7V9U reference using a single high-contrast highlight color. The design/fold triage section reuses baseline foldcheck_review PDBs for audit. The panel-selection section uses the expanded fold-check structure set and shows WT plus the selected six with pLDDT, RMSD, mutation count, MSA support, nucleic-acid-facing mutation count, charge change, and chemistry warnings beside the viewer. Raw local PDB files remain unchanged, and the browser view does not replace ColabFold fold validation |
...
| Panel-selection visuals | `../../../../src/dnadesign/studies/units/eco1_rt_repack/workspaces/eco1_rt_conservative_v1/outputs/thread/design_classes/selection/plots/` | materialized compact SVGs for gate counts by design class, full-population stratification with the selected six highlighted, selected-panel review axes, selected sequence differences, and selected mutation chemistry/geography; the review-deliverables notebook links these, the selected-panel table, the selected protein-sequence CSV, and the RT-only handoff boundary through the selection manifest |
| Atlas semantic context | `../contexts/fold-validation-policy.md`, `../../../../src/dnadesign/thread/adapters/esm_atlas/`, `../../../../src/dnadesign/thread/structure_predictions/`, and `../../../../src/dnadesign/studies/units/eco1_rt_repack/operations/materialization/atlas_semantic_profile/` | all-97 hash-lookup/on-demand probe materialized locally; WT is accepted with compact sparse SAE activation artifacts and one Atlas/ESMFold structure row, the first synthetic ProteinMPNN candidates still return explicit Atlas 404 rows, and the remaining synthetic rows are capped as unattempted |
| Biohub ESMC SAE profile | `../contexts/fold-validation-policy.md`, `../../../../src/dnadesign/thread/adapters/biohub_esmc/`, and `../../../../src/dnadesign/studies/units/eco1_rt_repack/operations/materialization/biohub_esmc_sae_profile/` | authenticated `/api/v1/encode` -> `/api/v1/logits` profile materialized locally for the baseline WT plus 96 candidates and for the expanded WT plus 576-candidate design-class pool; this is semantic annotation, not fold validation or candidate acceptance |
...
| ESMC sequence pseudo-likelihood | `../../../../src/dnadesign/permuter/src/scoring/esmc_pseudolikelihood/`, `../../../../src/dnadesign/thread/adapters/biohub_esmc/`, and `../../../../src/dnadesign/studies/units/eco1_rt_repack/operations/materialization/biohub_esmc_sequence_pseudolikelihood/` | deferred 6B leave-one-out sequence score. The method is documented and resumable, but whole-protein pseudo-likelihood is not required for the first assay panel |
...
| Candidate triage table | `../../../../src/dnadesign/studies/units/eco1_rt_repack/workspaces/eco1_rt_conservative_v1/outputs/thread/design_classes/selection/candidate_triage_table.parquet` | materialized table joining fold review, feasibility, MSA support, mutation geography, nucleic-acid-facing chemistry, sequence diversity, SAE-window fields, and ESMC additive LLR fields without reducing them to one combined rank |
