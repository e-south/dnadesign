...
| Panel-selection visuals | `../../../../src/dnadesign/studies/units/eco1_rt_repack/workspaces/eco1_rt_conservative_v1/outputs/thread/design_classes/selection/plots/` | materialized compact SVGs for `selection_premise_alignment`, `selection_design_class_gate_counts`, `selection_class_local_percentiles`, `selection_six_sequence_distance`, `selection_selected_substitutions_across_rt`, `selection_regional_mutation_burden`, `selection_local_structure_by_region`, and `selection_na_facing_chemistry_balance`; the review-deliverables notebook links these plus nonvisual evidence/export rows for the selected-panel table, local-structure metric table, selected protein-sequence CSV, and RT-only handoff readiness through the selection manifest |
