...
| Panel-selection visuals | `../../../../src/dnadesign/studies/units/eco1_rt_repack/workspaces/eco1_rt_conservative_v1/outputs/thread/design_classes/selection/plots/` | materialized compact SVGs for `selection_design_class_gate_counts`, `selection_population_stratification`, `selection_class_local_percentiles`, `selection_six_sequence_distance`, `selection_selected_substitutions_across_rt`, `selection_regional_mutation_burden`, and `selection_na_facing_chemistry_balance`; the review-deliverables notebook links these, the selected-panel table, the selected protein-sequence CSV, and the RT-only handoff boundary through the selection manifest |
