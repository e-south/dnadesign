...
| Review deliverables | `../../../src/dnadesign/studies/units/eco1_rt_repack/operations/materialization/review_deliverables/` and `../../../src/dnadesign/studies/units/eco1_rt_repack/workspaces/eco1_rt_conservative_v1/outputs/thread/review_deliverables/` | materialized foundation bundle; writes a manifest-relative visual manifest, a Mestre-derived clade 9 scaffold/mask-evidence panel, linear mask tracks, ChimeraX mask-context script/render, ProteinMPNN diversity plots, a Tao-style ColabFold RMSD/pLDDT joint plot for the current single mask policy, linked fold-review SVG/PNG visuals, linked WT ESMC masked-marginal plots, MSA-vs-ESMC model-constraint audit plots with correlation evidence, exact-dictionary Biohub ESMC SAE interpretation plots, a joint SAE-similarity/ColabFold-pLDDT/ESMC-single-substitution-LLR review panel, an interactive structure-browser manifest that reuses local foldcheck_review PDBs through `dnadesign.thread.structure_views`, and a manifest-backed marimo notebook organized by progressive analysis sections with fitted visual previews. ChimeraX remains the still-render and pose-capture path. WT Biohub ESMC SAE structure frames, feature-window heatmaps, and feasibility/selection matrices remain planned follow-on surfaces |
