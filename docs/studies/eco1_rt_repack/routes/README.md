...
| Planned generation-policy cleanup dev spec | `../contexts/generation-policy-cleanup-dev-spec.md` |
...
| Planned generation-policy cleanup | `../contexts/generation-policy-cleanup-dev-spec.md`, `../../../../src/dnadesign/studies/units/eco1_rt_repack/operations/materialization/generation_policies/`, and `../../../../docs/bu-scc/jobs/eco1-proteinmpnn-generation-policy.qsub` | planned breaking cleanup; replaces the current nested distance-mask family with complete ProteinMPNN generation policies. v2 policy/request materialization and SCC request execution wiring are staged, but ProteinMPNN outputs, fold checks, selection, and review regeneration are not accepted current runtime behavior |
...
| Expanded fold-check review | `../../../../src/dnadesign/studies/units/eco1_rt_repack/workspaces/eco1_rt_conservative_v1/outputs/thread/design_classes/foldcheck_review/foldcheck_candidate_ranking.parquet` | materialized expanded review ranking for 576 candidates; use this before feasibility and global panel selection |
...
| Interactive browser structure review | `../../../../src/dnadesign/thread/structure_views/` and `../../../../src/dnadesign/studies/units/eco1_rt_repack/workspaces/eco1_rt_conservative_v1/outputs/thread/review_deliverables/structure_browser/` | materialized interactive inspection for the Ec86 reference and fitted fold models; uses py3Dmol through the generic `dnadesign.thread.structure_views` contract. The constraint-evidence section highlights one selected mask-evidence category at a time on the ec86kit/7V9U reference using a single high-contrast highlight color. The design/fold triage section reuses baseline foldcheck_review PDBs for audit. The panel-selection section uses the expanded fold-check structure set and shows WT plus the selected primary panel with pLDDT, RMSD, mutation count, MSA support, mutation count near retained DNA/RNA or thumb-track positions, charge change, and chemistry warnings beside the viewer. Structure group labels are section-specific, and molecule-visibility controls stay stable while moving between visuals. Raw local PDB files remain unchanged, and the browser view does not replace ColabFold fold validation |
...
| SAE window summary | `../../../../src/dnadesign/studies/units/eco1_rt_repack/operations/materialization/sae_window_summary/` and `../../../../src/dnadesign/studies/units/eco1_rt_repack/workspaces/eco1_rt_conservative_v1/outputs/thread/design_classes/biohub_esmc/sae_feature_window_summary.parquet` | materialized three-window WT-delta summary for 577 sequences across catalytic-palm control, thumb/palm nucleic-acid-binding surface, and mutable near retained DNA/RNA/basic-surface windows; this is review evidence, not an acceptance gate or selector |
...
| Selection readiness | `../../../../src/dnadesign/studies/units/eco1_rt_repack/operations/materialization/selection_readiness/` and `../../../../src/dnadesign/studies/units/eco1_rt_repack/workspaces/eco1_rt_conservative_v1/outputs/thread/design_classes/selection/selection_readiness_manifest.yaml` | materialized feasibility, triage, local-structure region metrics, primary-panel trace, and primary conservative panel for the expanded candidate pool; MSA support, mutation geography, local chemistry, local/global fold metrics, and sequence nonredundancy explain panel choices |
| Candidate triage table | `../../../../src/dnadesign/studies/units/eco1_rt_repack/workspaces/eco1_rt_conservative_v1/outputs/thread/design_classes/selection/candidate_triage_table.parquet` | materialized table joining fold review, feasibility, MSA support, mutation geography, near retained DNA/RNA or thumb-track chemistry, local-structure gate fields, sequence diversity, SAE-window fields, and ESMC additive LLR fields without reducing them to one combined rank |
| Candidate selection panel | `../../../../src/dnadesign/studies/units/eco1_rt_repack/workspaces/eco1_rt_conservative_v1/outputs/thread/design_classes/selection/candidate_selection_panel.parquet` | materialized primary panel selected globally after preservation and chemistry/support gates; not an activity ranking and not selected by ESMC or SAE |
| Selected protein sequence export | `../../../../src/dnadesign/studies/units/eco1_rt_repack/workspaces/eco1_rt_conservative_v1/outputs/thread/design_classes/selection/candidate_handoff_sequences.csv` | flat RT-only protein sequence table for the selected primary panel; it carries sequence hashes and explicit not-materialized DNA-design/restriction-screen status, and it is the sequence authority for later handoff planning |
