...
| `infer_sequence_view_completion` | `infer validate sequence-view-completion --format json` classifies sequence-view feature work before model execution | planner command failed, output was malformed, or reusable/stale/missing thresholds were exceeded | command surface plus optional max missing/stale/product thresholds | cheap-to-medium | separate missing feature vectors from missing sequence products before Evo2 backfill |
...
| `sequence_view_contract` | a USR dataset's `_views/sequence_views.parquet` sidecar satisfies declared product-kind, orientation, pooling, length, and bounds expectations | sidecar missing, stale semantic names, invalid bounds, or wrong counts | dataset artifact id plus expected sidecar counts | cheap-to-medium | prove construct-ready anchors and forward/reverse-complement contexts are present before Infer |
