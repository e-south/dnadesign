...
| Run the deeper promoter-study command preflight | [Promoter study preflight contract](../src/dnadesign/usr/docs/operations/promoter-study-preflight.md) | Start from the active study selector, use `ops.study.yaml` for phase and scope posture, and use `pipeline.yaml` when present for the exact Construct, Infer, Notify, and runbook paths. |
