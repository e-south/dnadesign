...
| Run the deeper promoter-study command preflight | [Promoter study preflight contract](../src/dnadesign/usr/docs/operations/promoter-study-preflight.md) | Start from the active study selector, then use the checked-in `pipeline.yaml` for Construct, Infer, Notify, and runbook paths instead of reconstructing them from generic docs. |
