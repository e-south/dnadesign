...
| What blocks conservation masking? | `src/dnadesign/studies/units/eco1_rt_repack/operations/materialization/conservation/` and `contexts/msa-method.md` | The materializer exists, but real aligned FASTA inputs from the selected source contract are still missing. |
...
| Should this use `aligner`? | Dev spec open questions and mask policy | Protein MSA/conservation is not hidden inside current Aligner without a modernization contract. |
