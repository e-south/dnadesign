...
| What blocks conservation masking? | `src/dnadesign/studies/units/eco1_rt_repack/operations/materialization/source_sequences/provider_sources/`, `source_sequences/roster_cache/`, `source_sequences/`, `conservation_alignments/`, `conservation/`, `src/dnadesign/aligner/msa/`, `src/dnadesign/aligner/msa/visualization/`, `workbench/ontology/rt-annotation-tracks.yaml`, `workbench/ontology/msa-exemplar-rows.yaml`, `workbench/ontology/msa-panel-spec.yaml`, and `contexts/msa-method.md` | Provider-source, roster-cache, source-sequence, source-bundle sufficiency, conservation-alignment, conservation-profile, generic MAFFT bundle, generic MSA visualization, Eco1-owned motif annotation, Eco1-owned exemplar-row, and Eco1-owned panel-display seams exist. The selected broad profile is now `broad_tao_homolog_rt`; full Mestre rows are candidate/context only. Bounded broad source records are not materialized yet; the selected `eco1_like_retron_rt` alignment and partial generic visualization are accepted locally; the former full-roster broad MAFFT run was stopped after roughly four hours, so the complete aligned FASTA bundle and conservation profile are still missing. |
