...
| What blocks conservation masking? | `src/dnadesign/studies/units/eco1_rt_repack/operations/materialization/source_sequences/provider_sources/`, `source_sequences/roster_cache/`, `source_sequences/`, `conservation_alignments/`, `conservation/`, `src/dnadesign/aligner/msa/`, `src/dnadesign/aligner/msa/visualization/`, `workbench/ontology/rt-annotation-tracks.yaml`, `workbench/ontology/msa-exemplar-rows.yaml`, `workbench/ontology/msa-panel-spec.yaml`, and `contexts/msa-method.md` | Provider-source, roster-cache, source-sequence, source-bundle sufficiency, conservation-alignment, conservation-profile, generic MSA bundle, generic MSA visualization, Eco1-owned motif annotation, Eco1-owned exemplar-row, and Eco1-owned panel-display seams exist. The selected broad profile is now `ec86_clade9_conservation_v1`, the Mestre Ec86 RT clade 9 conservation panel; full Mestre rows are candidate/context only. Clade 9 source records and source FASTA sufficiency now pass locally after declared QC, with 302 included clade-9 rows and 44 included II-A3/42_1 rows. The prior `ec86_iia3_cluster42_1_conservation_v1` alignment and partial generic visualization are historical local evidence until regenerated under Clustal Omega; the accepted two-profile aligned FASTA bundle and conservation profile are still missing. |
