...
| Conservation routing | What blocks conservation masking? | Route to the MSA method, conservation source contract, roster-cache materializer, source-sequence materializer plus sufficiency gate, `aligner.msa` for generic aligned FASTA bundles, and the conservation materializer. | Pass if the answer says real Mestre/provider source inputs, sufficiency-passing source FASTA, aligned FASTA inputs, and conservation profile are still missing and does not imply live fetch/provider fallback or conservation scoring inside aligner. |
