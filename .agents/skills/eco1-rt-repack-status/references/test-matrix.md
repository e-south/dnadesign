...
| Conservation routing | What blocks conservation masking? | Route to the MSA method, conservation source contract, provider-source materializer, roster-cache materializer, source-sequence materializer plus sufficiency gate, `aligner.msa` for generic aligned FASTA bundles, and the conservation materializer. | Pass if the answer says local source FASTA bundles pass sufficiency while aligned FASTA inputs and conservation profile are still missing, and does not imply source authority, provider ledgers, or conservation scoring inside aligner. |
...
| Source/test IA regression | Add a flat operation module, flat primitive materializer, flat materialization test, or root-level source-sequence subprimitive test. | Audit fails before the status skill can be treated as healthy. | Pass if only semantic `operations/contracts/`, `operations/materialization/<primitive>/`, `tests/contracts/`, `tests/materialization/<primitive>/`, and mirrored `tests/materialization/source_sequences/<subprimitive>/` packages are accepted. |
