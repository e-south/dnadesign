...
| Functional edge | GUI close with a stale or malformed PID. | Run the stop helper with `--close-gui`. | Pass if non-positive, dead, non-ChimeraX, or wrong-port process IDs fail without receiving a signal. |
