...
| "I have a profile but not bases." | scar-nick route in `routes/README.md` / `routes/scar-nick-base-junction.md` | Base feasibility is a primitive search problem. |
| "I need a cap or shortening geometry." | released-product Snapback route in `routes/README.md` / `routes/released-product-snapback.md` | Cap/shortening is solved by Snapback, not the compiler. |
| "I need mismatch/boundary illustration." | YIU route in `routes/README.md` / `routes/yiu-boundary-check.md` | YIU is contrast-only. |
...
- Use `operations/pipeline.yaml` only when a machine-readable command group is needed.
