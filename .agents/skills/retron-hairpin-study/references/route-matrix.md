...
| "I have a profile but not bases." | scar-nick route in `routes.md` / `routes/scar-nick-base-junction.md` | Base feasibility is a primitive search problem. |
| "I need a cap or shortening geometry." | released-product Snapback route in `routes.md` / `routes/released-product-snapback.md` | Cap/shortening is solved by Snapback, not the compiler. |
| "I need mismatch/boundary illustration." | YIU route in `routes.md` / `routes/yiu-boundary-check.md` | YIU is contrast-only. |
