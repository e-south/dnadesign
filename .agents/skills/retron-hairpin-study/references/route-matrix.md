...
| --- | --- | --- |
| I just started a fresh thread or the skill is not visible. | `docs/studies/README.md`, then `cruncher.data-plane.cruncher-study-status --study-dir docs/studies/retron_hairpin_design --json` | The direct command path is canonical even when project-scope skill discovery is absent. |
...
| I need to harden the study status, preflight, or native-agent bootstrap. | `retron-hairpin-study` plus `harness-engineering` | This is a harness problem, not just a prose problem. |
