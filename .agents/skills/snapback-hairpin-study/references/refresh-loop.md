...
Start with the checked-in shortening study record, then refresh only the
...
2. Read `docs/studies/snapback_shortening_effort/status.md`.
3. Run
...
- Pair with `pragmatic-programming-principles` when the change touches lane
  boundaries, explicit contracts, degraded modes, or no-silent-fallback rules.

## Failure routing

- Missing study record or stale selector: repair `docs/studies/index.yaml` or
  the `docs/studies/snapback_shortening_effort/` bundle, then rerun status.
