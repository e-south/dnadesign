...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/junction/contracts/request/codec.py

Strict mapping parser and stable serializer for junction requests.
...
_TARGET_FIELDS = frozenset({"id", "assembly_group_id", "sequence", "recovery_primers"})
...
def _parse_targets(raw: object) -> tuple[Target, ...]:
    if not isinstance(raw, list):
        raise JunctionConfigError("targets must be a list")
    targets: list[Target] = []
    for index, target_value in enumerate(raw):
        ...
        targets.append(
            Target(
                ...
                assembly_group_id=require_identifier(
                    target_raw["assembly_group_id"], context=f"{context}.assembly_group_id"
                ),
...
def _parse_order_policy(raw: object) -> OrderPolicy:
    ...
    if not isinstance(complement_end_preparation, str):
        raise JunctionConfigError("order_policy.complement_end_preparation must be a string")
    return OrderPolicy(
...
def parse_request(raw: Mapping[str, object]) -> JunctionRequest:
    ...
    if not isinstance(schema, str):
        raise JunctionConfigError("schema must be a string")
...
def request_to_mapping(request: JunctionRequest) -> dict[str, object]:
...
def canonical_request_bytes(request: JunctionRequest) -> bytes:
    ...
    if len(content) > MAX_REQUEST_BYTES:
        raise JunctionConfigError(
            "junction canonical request exceeds the "
