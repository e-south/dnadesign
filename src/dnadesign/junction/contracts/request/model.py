...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/junction/contracts/request/model.py

Frozen values for the canonical junction request.
    ...
    def __post_init__(self) -> None:
        ...
        for field_name, maximum in _PLANNING_BUDGET_CEILINGS.items():
            if getattr(self, field_name) > maximum:
                raise JunctionConfigError(f"planning.{field_name} must not exceed {maximum} for {REQUEST_SCHEMA}")
        object.__setattr__(
            self,
            "barcode_gc_min",
        ...
        if self.barcode_gc_min > self.barcode_gc_max:
            raise JunctionConfigError("planning.barcode_gc_min must not exceed barcode_gc_max")
        if self.barcode_max_homopolymer > self.barcode_length:
            raise JunctionConfigError("planning.barcode_max_homopolymer must not exceed barcode_length")
        if self.barcode_pool_factor < 5:
            raise JunctionConfigError("planning.barcode_pool_factor must be at least 5")
        if self.toehold_length < 2:
            raise JunctionConfigError("planning.toehold_length must be at least 2")
        if self.barcode_toehold_k > min(self.barcode_length, self.toehold_length):
            raise JunctionConfigError("planning.barcode_toehold_k must not exceed barcode_length or toehold_length")
        if self.barcode_pair_k > self.barcode_length:
            raise JunctionConfigError("planning.barcode_pair_k must not exceed barcode_length")
...
class RecoveryPrimerPair:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.mode, str) or self.mode not in RECOVERY_PRIMER_MODES:
            ...
            raise JunctionConfigError(f"recovery_primers.mode must be one of: {allowed}")
        if not isinstance(self.forward, Primer):
            raise JunctionConfigError("recovery_primers.forward must be a Primer value")
        if not isinstance(self.reverse, Primer):
            raise JunctionConfigError("recovery_primers.reverse must be a Primer value")


@dataclass(frozen=True, slots=True)
class Target:
    """One exact linear DNA target and its assembly-group identity."""
    ...
    def __post_init__(self) -> None:
        ...
        require_identifier(self.assembly_group_id, context="target.assembly_group_id")
        require_dna(self.sequence, context=f"target {self.id!r} sequence")
        if not isinstance(self.recovery_primers, RecoveryPrimerPair):
            raise JunctionConfigError("target.recovery_primers must be a RecoveryPrimerPair value")
        if not self.sequence.startswith(self.recovery_primers.forward.binding_sequence):
            raise JunctionConfigError(f"target {self.id!r} recovery forward primer must match the target prefix")
...
class JunctionRequest:
    """Canonical, validated junction planning request."""
    ...
    def __post_init__(self) -> None:
        if self.schema != REQUEST_SCHEMA:
            raise JunctionConfigError(f"schema must equal {REQUEST_SCHEMA!r}")
        require_int(self.seed, context="seed", minimum=0)
        if not isinstance(self.planning, PlanningProfile):
            raise JunctionConfigError("planning must be a PlanningProfile value")
        if not isinstance(self.order_policy, OrderPolicy):
            raise JunctionConfigError("order_policy must be an OrderPolicy value")
        ...
        ids: set[str] = set()
        grouped_sequences: set[tuple[str, str]] = set()
        for target in self.targets:
            if target.id in ids:
                raise JunctionConfigError(f"duplicate target id: {target.id!r}")
            ids.add(target.id)
            grouped_key = (target.assembly_group_id, target.sequence)
            if grouped_key in grouped_sequences:
                raise JunctionConfigError(f"duplicate sequence within assembly group {target.assembly_group_id!r}")
            grouped_sequences.add(grouped_key)
