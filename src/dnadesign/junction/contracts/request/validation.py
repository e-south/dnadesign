...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/junction/contracts/request/validation.py

Primitive validation rules for the junction request boundary.
...
def require_exact_fields(
    ...
    *,
    ...
    if unknown := sorted(keys - required):
        raise JunctionConfigError(f"{context} contains unknown field(s): {', '.join(unknown)}")
    if missing := sorted(required - keys):
        raise JunctionConfigError(f"{context} is missing field(s): {', '.join(missing)}")
...
def require_mapping(value: object, *, context: str) -> Mapping[str, object]:
    ...
    if not isinstance(value, Mapping) or any(not isinstance(key, str) for key in value):
        raise JunctionConfigError(f"{context} must be an object with string keys")
    return value
...
def require_nonempty_string(value: object, *, context: str) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        raise JunctionConfigError(f"{context} must be a non-empty string")
    if value != value.strip():
        raise JunctionConfigError(f"{context} must not contain leading or trailing whitespace")
    return value
...
def require_identifier(value: object, *, context: str) -> str:
    ...
    if len(identifier.encode("ascii")) > MAX_REQUEST_IDENTIFIER_BYTES:
        raise JunctionConfigError(f"{context} must not exceed {MAX_REQUEST_IDENTIFIER_BYTES} ASCII bytes")
    return identifier
...
def require_plain_text(value: object, *, context: str) -> str:
    ...
    if any(ord(character) < 32 or ord(character) == 127 for character in text):
        raise JunctionConfigError(f"{context} must not contain control characters")
    if text[0] in "=+-@":
        raise JunctionConfigError(f"{context} must not begin with a spreadsheet formula marker")
    if len(text.encode("utf-8")) > MAX_REQUEST_PLAIN_TEXT_BYTES:
        raise JunctionConfigError(f"{context} must not exceed {MAX_REQUEST_PLAIN_TEXT_BYTES} UTF-8 bytes")
    return text
...
def require_int(value: object, *, context: str, minimum: int) -> int:
    ...
    if isinstance(value, bool) or not isinstance(value, int) or value < minimum:
        ...
        raise JunctionConfigError(f"{context} must be a {qualifier} integer")
    if value > MAX_REQUEST_INTEGER:
        raise JunctionConfigError(f"{context} must not exceed {MAX_REQUEST_INTEGER}")
    return value
...
def require_fraction(value: object, *, context: str) -> float:
    ...
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise JunctionConfigError(f"{context} must be a number between 0 and 1")
    try:
        result = float(value)
    except OverflowError:
        raise JunctionConfigError(f"{context} must be between 0 and 1") from None
    if not 0.0 <= result <= 1.0:
        raise JunctionConfigError(f"{context} must be between 0 and 1")
    return result
...
def require_dna(value: object, *, context: str) -> str:
    ...
    if not isinstance(value, str) or not value or not set(value) <= DNA_ALPHABET:
        raise JunctionConfigError(f"{context} must be a non-empty uppercase ACGT sequence")
    return value
...
def require_optional_dna(value: object, *, context: str) -> str:
    ...
    if not isinstance(value, str) or not set(value) <= DNA_ALPHABET:
        raise JunctionConfigError(f"{context} must be an uppercase ACGT sequence or an empty string")
    return value
...
def parse_recovery_primer_mode(value: object, *, context: str) -> str:
    ...
    if not isinstance(value, str) or value not in RECOVERY_PRIMER_MODES:
        ...
        raise JunctionConfigError(f"{context} must be one of: {allowed}")
    return value
