...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/junction/design/planner.py

Deterministic orchestration of the pure junction design stages.
...
from dnadesign.junction.contracts.identity import mapping_sha256
from dnadesign.junction.contracts.plan import (
    AssemblyGroupPlan,
    AssemblyGroupSearchEvidence,
    CheckResult,
    CheckSubject,
    JunctionPlan,
    SelectedJunction,
...
from dnadesign.junction.design.resources.artifact_projection import (
...
_PLAN_SCHEMA = "dnadesign.junction.plan.v1"


def design_junction(request: JunctionRequest) -> JunctionPlan:
    """Compile one trusted request into an immutable in-memory plan."""

    return _compile_junction(request, pipeline=_STRING_SEARCH_V1)


def _predict_loci(request: JunctionRequest) -> dict[str, int]:
...
def _compile_junction(
    ...
    *,
    ...
    assembly_group_locus_counts_by_id: dict[str, int] = defaultdict(int)
    input_bases = 0
    for target in request.targets:
        locus_count = predicted_loci_by_target[target.id]
        assembly_group_locus_counts_by_id[target.assembly_group_id] += locus_count
    ...
    request_workload = estimate_request_workload(
        ...
        assembly_group_locus_counts=tuple(
            assembly_group_locus_counts_by_id[assembly_group_id]
            for assembly_group_id in sorted(assembly_group_locus_counts_by_id)
        ),
        profile=profile,
    )
    ...
    targets_by_assembly_group: dict[str, list] = defaultdict(list)
    for target in request.targets:
        targets_by_assembly_group[target.assembly_group_id].append(target)

    assembly_group_plans: list[AssemblyGroupPlan] = []
    assignments_by_assembly_group: dict[str, tuple[JunctionAssignment, ...]] = {}
    checks: list[CheckResult] = []
    for assembly_group_id in sorted(targets_by_assembly_group):
        targets = tuple(sorted(targets_by_assembly_group[assembly_group_id], key=lambda target: target.id))
        guard_uniform_toehold_search(
        ...
        barcode_subset_seed = derive_seed(request.seed, assembly_group_id=assembly_group_id, stage="barcode-subsets")
        matching_seed = derive_seed(request.seed, assembly_group_id=assembly_group_id, stage="matching")
        ...
        assignments_by_assembly_group[assembly_group_id] = assignments
        ...
        checks.extend(
            (
                CheckResult(
                    check="assembly_group_one_to_one_matching",
                    status="passed",
                    subject=CheckSubject(kind="assembly_group", id=assembly_group_id),
    ...
    target_plans = []
    orders = []
    for target in sorted(request.targets, key=lambda item: (item.assembly_group_id, item.id)):
        composition = compose_target(
            target,
            assignments_by_assembly_group[target.assembly_group_id],
    ...
    provisional = JunctionPlan(
        schema=_PLAN_SCHEMA,
        algorithm=pipeline.algorithm_id,
        request_sha256=request_sha256,
        plan_id="",
        seed=request.seed,
        assembly_groups=tuple(assembly_group_plans),
