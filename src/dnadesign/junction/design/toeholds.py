...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/junction/design/toeholds.py
...
from dnadesign.junction.sequence import POSITION_WEIGHT_SCALE, validate_dna
    ...
    *,
    ...
    try:
        ...
        raise JunctionDesignError(
            ...
            "toehold_search_iterations, or the number of loci in the assembly group."
        ) from error
...
class _PairDistanceCache:
    """Lazy compact symmetric matrix for one assembly group's candidates."""
    ...
    def __init__(self, candidates: tuple[ToeholdCandidate, ...]) -> None:
        ...
        if len(sequence_lengths) != 1:
            raise JunctionDesignError("Toehold candidates in one assembly group must have one length.")
        ...
        if required_bytes > MAX_TOEHOLD_CACHE_BYTES:
            raise JunctionDesignError(
                ...
                "Reduce search_range, or use separate assembly_group_id values only when fragments will not "
...
def _selection_pair_blocks(
    ...
    *,
    ...
    if cells_per_trial < 1:
        raise JunctionDesignError("Toehold selection cannot fit one trial block in its memory envelope.")
...
def select_toeholds(
    ...
    *,
    ...
    if not ordered_loci:
        raise JunctionDesignError("An assembly group has no toehold loci to design.")
    candidate_counts = tuple(len(locus.candidates) for locus in ordered_loci)
    if len(set(candidate_counts)) != 1:
        raise JunctionDesignError("Every toehold locus in one assembly group must have one candidate count.")
    guard_uniform_toehold_search(
