...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/junction/design/resources/guards.py
...
from .limits import (
    ...
    MAX_REQUEST_ASSEMBLY_GROUP_COUNT,
...
_REQUEST_WORKLOAD_LIMITS = (
    ("assembly groups", "assembly_group_count", MAX_REQUEST_ASSEMBLY_GROUP_COUNT),
...
def guard_request_workload(estimate: RequestWorkloadEstimate) -> None:
    """Reject unsafe aggregate work before any assembly-group search."""
    ...
    for label, field_name, limit in _REQUEST_WORKLOAD_LIMITS:
        ...
        if requested > limit:
            raise JunctionDesignError(
                ...
                f"requested {requested}, limit {limit}. Split genuinely independent assembly groups across requests "
    ...
    *,
    ...
    if encoded_bases > MAX_TOEHOLD_ENCODED_BASES:
        raise JunctionDesignError(
            ...
            f"limit {MAX_TOEHOLD_ENCODED_BASES}. Reduce toehold_length, or use separate assembly_group_id values only "
            "when fragments will not encounter one another during the intended three-way-junction assembly."
        )
    ...
    if required_bytes > MAX_TOEHOLD_CACHE_BYTES:
        raise JunctionDesignError(
            ...
            f"limit {MAX_TOEHOLD_CACHE_BYTES}. Reduce search_range, or use separate assembly_group_id values only "
            "when fragments will not encounter one another during the intended three-way-junction assembly."
        )
    ...
    if requested_lookups > MAX_TOEHOLD_DISTANCE_LOOKUPS:
        raise JunctionDesignError(
            ...
            "Lower the declared toehold_search_iterations, or use separate assembly_group_id values only when "
            "fragments will not encounter one another during the intended three-way-junction assembly; junction does "
            "not run an unbounded search."
        )
    ...
    *,
    ...
    if modeled_state_bytes > MAX_BARCODE_GENERATION_STATE_BYTES:
        raise JunctionDesignError(
            ...
            f"limit {MAX_BARCODE_GENERATION_STATE_BYTES}. Reduce barcode_pool_factor, or use separate "
            "assembly_group_id values only when fragments will not encounter one another during the intended "
            "three-way-junction assembly."
        )
    ...
    *,
    ...
    if lookups > MAX_BARCODE_SUBSET_LOOKUPS:
        raise JunctionDesignError(
            ...
            f"limit {MAX_BARCODE_SUBSET_LOOKUPS}. Lower barcode_subset_iterations, or use separate assembly_group_id "
            "values only when fragments will not encounter one another during the intended three-way-junction assembly."
        )
...
def guard_matching_search(*, count: int, combined_length: int, evaluations: int) -> None:
    ...
    if substring_character_visits > MAX_MATCHING_SUBSTRING_CHARACTER_VISITS:
        raise JunctionDesignError(
            ...
            "matching_iterations, toehold_length, or barcode_length; junction does not run an "
            "unbounded search."
        )
