...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/junction/design/resources/estimates.py
...
if TYPE_CHECKING:
    from dnadesign.junction.contracts.request import PlanningProfile
...
def estimate_request_workload(
    *,
    input_bases: int,
    target_count: int,
    assembly_group_locus_counts: tuple[int, ...],
    profile: PlanningProfile,
) -> RequestWorkloadEstimate:
    """Estimate request-wide work using the same bounds as each stage guard."""

    if input_bases < 0 or target_count < 0 or any(count < 0 for count in assembly_group_locus_counts):
        raise ValueError("request workload inputs must be non-negative")

    total_loci = sum(assembly_group_locus_counts)
    ...
    for locus_count in assembly_group_locus_counts:
        ...
        assembly_group_subset_lookups = (profile.barcode_subset_iterations + 1) * subset_pairs
        barcode_subset_lookups += assembly_group_subset_lookups
        barcode_dp_cells += (
            min(barcode_pairs, assembly_group_subset_lookups) * profile.barcode_length * profile.barcode_length
        )
    ...
    return RequestWorkloadEstimate(
        assembly_group_count=len(assembly_group_locus_counts),
        ...
        barcode_generation_base_visits=(
            len(assembly_group_locus_counts) * profile.barcode_generation_attempts * profile.barcode_length
        ),
