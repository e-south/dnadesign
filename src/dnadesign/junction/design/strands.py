...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/junction/design/strands.py

Paper-grounded strand composition and exact reconstruction checks.
...
def _recovery_evidence_and_orders(
    ...
    *,
    ...
    if extended_bottom != reverse_complement(extended_top):
        raise JunctionDesignError(f"Target '{target.id}' recovery products are not exact reverse complements.")
    evidence = RecoveryEvidence(
        ...
        expected_unextended_target=target.sequence,
...
def compose_target(
    ...
    *,
    ...
    if not target_assignments:
        raise JunctionDesignError(f"Three-way-junction target '{target.id}' has no selected junctions.")
    ...
    for left, right in zip(target_assignments, target_assignments[1:], strict=False):
        if left.candidate.start + toehold_length > right.candidate.start:
            raise JunctionDesignError(f"Target '{target.id}' has overlapping selected toeholds.")
    ...
    reconstructed_complement = "".join(fragment.complement_strand for fragment in reversed(fragments))
    if reconstructed != target.sequence:
        raise JunctionDesignError(f"Target '{target.id}' failed exact sequence reconstruction.")
    if reconstructed_complement != reverse_complement(target.sequence):
        raise JunctionDesignError(f"Target '{target.id}' failed complement-sequence reconstruction.")
    ...
    plan = TargetPlan(
        ...
        reconstructed_complement=reconstructed_complement,
    )
    checks = (
        CheckResult(
        ...
        CheckResult(
            check="complement_sequence_reconstruction",
            status="passed",
            subject=CheckSubject(kind="target", id=target.id),
            detail="reverse-order complement strands reconstruct rc(target)",
        ),
        CheckResult(
            check="complement_nick_sequence_layout",
            status="passed",
            subject=CheckSubject(kind="target", id=target.id),
            detail="complement-strand nick layout is internally consistent; chemistry was not evaluated",
        ),
        CheckResult(
            check="terminal_primer_sequence_match",
            status="passed",
            subject=CheckSubject(kind="target", id=target.id),
