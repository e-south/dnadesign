...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/junction/tests/test_hardening_contracts.py

Adversarial contracts at junction's public and publication boundaries.
...
def test_three_way_junction_rejects_targets_below_junction_geometry() -> None:
        ...
        _request_mapping(
            targets=[
                _target(
                    ...
                    assembly_group_id="junction-group",
                    sequence=sequence,
                )
            ]
        )
    ...
    with pytest.raises(JunctionDesignError, match="no complete toehold locus"):
        plan(request)


def test_barcode_ids_are_assembly_group_qualified_and_globally_unique() -> None:
    targets = [
        _target(
            target_id="target-a",
            assembly_group_id="assembly-a",
    ...
    barcode_ids = [
        junction.barcode_id for assembly_group in result.assembly_groups for junction in assembly_group.junctions
    ]

    assert barcode_ids
    assert len(barcode_ids) == len(set(barcode_ids))
    assert all(
        junction.barcode_id.startswith(f"{assembly_group.assembly_group_id}:barcode-")
        for assembly_group in result.assembly_groups
        for junction in assembly_group.junctions
    )
...
def test_verification_rejects_undeclared_entries(
    ...
    with pytest.raises(JunctionBundleError, match=match):
        verify(destination)
...
def test_verification_rejects_artifact_replaced_by_symlink_before_open(
    ...
    with pytest.raises(JunctionBundleError):
        verify(destination)
...
def test_verification_rejects_undeclared_file_added_after_initial_inventory(
    ...
    with pytest.raises(JunctionBundleError, match="undeclared file"):
        verify(destination)
...
def test_public_dataclass_construction_rejects_untrimmed_strings() -> None:
    ...
    with pytest.raises(JunctionConfigError, match="leading or trailing whitespace"):
        replace(target, id=f" {target.id}")
    with pytest.raises(JunctionConfigError, match="leading or trailing whitespace"):
        replace(target, assembly_group_id=f"{target.assembly_group_id} ")
    with pytest.raises(JunctionConfigError, match="leading or trailing whitespace"):
        replace(request.order_policy, synthesis_scale=f" {request.order_policy.synthesis_scale}")
    with pytest.raises(JunctionConfigError):
...
def test_complete_plan_is_stable_across_hash_seeds_and_fresh_processes(tmp_path: Path) -> None:
    ...
    code = (
        "import json, sys; "
        "from dnadesign.junction import plan; "
        "from dnadesign.junction.contracts.request import load_request; "
