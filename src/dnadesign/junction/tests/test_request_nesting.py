...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/junction/tests/test_request_nesting.py
...
from dnadesign.junction.contracts.request.limits import MAX_REQUEST_BYTES
...
def test_load_request_rejects_excessive_document_nesting_as_config_error(
    ...
    with pytest.raises(
        JunctionConfigError,
        match=rf"^Invalid {format_name} in junction request: ",
    ) as caught:
...
def test_cli_reports_excessive_document_nesting_as_sanitized_json(
    ...
    assert payload["error"] == {
        "code": "config_error",
        "message": f"Invalid {suffix[1:].upper()} in junction request: {request}",
        "retryable": False,
    }
