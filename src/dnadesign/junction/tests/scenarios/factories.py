...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/junction/tests/scenarios/factories.py
...
def deterministic_dna(label: str, length: int) -> str:
    ...
    for retry in range(10_000):
        digest = shake_256(
            b"dnadesign.junction.dogfood.target.v1\0"
            + label.encode()
            + b"\0"
            + str(length).encode()
            + b"\0"
...
def scale_request_mapping(
    *,
    ...
    for index in range(target_count):
        ...
        assembly_group_id = "assembly-shared" if topology == "shared" else f"assembly-{index:04d}"
        assembly_group_sequences = used_sequences.setdefault(assembly_group_id, set())
        assembly_group_recovery_pairs = used_recovery_pairs.setdefault(assembly_group_id, set())
        for collision_retry in range(10_000):
            ...
            if sequence not in assembly_group_sequences and recovery_pair not in assembly_group_recovery_pairs:
                assembly_group_sequences.add(sequence)
                assembly_group_recovery_pairs.add(recovery_pair)
                break
        else:
