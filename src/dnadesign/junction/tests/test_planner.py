...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/junction/tests/test_planner.py

End-to-end contracts for the pure junction planner.
...
def test_design_is_deterministic_and_reconstructs_the_exact_target() -> None:
    ...
    first = design_junction(request)
    second = design_junction(request)

    assert first == second
    assert first.plan_id.startswith("sha256:")
    target = first.targets[0]
    assert target.reconstructed_target == request.targets[0].sequence
    assert target.reconstructed_complement == reverse_complement(request.targets[0].sequence)
    assert len(target.fragments) == len(target.junctions) + 1
    assert [target.fragments[0].role, target.fragments[-1].role] == ["first", "last"]
    assert all(junction.complement_nick_sequence_layout_valid for junction in target.junctions)
...
def test_fragment_strands_follow_the_paper_defined_equations() -> None:
    ...
    selected = sorted(
        (junction for junction in result.assembly_groups[0].junctions if junction.target_id == target.target_id),
...
def test_target_input_order_does_not_change_the_plan() -> None:
    ...
    forward_mapping["planning"]["barcode_pair_k"] = 6
    reverse_mapping["planning"]["barcode_pair_k"] = 6
    forward = design_junction(parse_request(forward_mapping))
    reverse = design_junction(parse_request(reverse_mapping))

    assert forward == reverse


def test_unrelated_assembly_group_does_not_perturb_existing_assembly_group_search() -> None:
    baseline = design_junction(parse_request(_request_mapping()))
    first_sequence = _target_sequence(offset=0)
    second_sequence = _target_sequence(offset=3)
    targets = [
        {
        ...
        for target_id, assembly_group_id, sequence in (
            ("target-a", "assembly-a", first_sequence),
            ("target-b", "assembly-b", second_sequence),
        )
    ]

    extended = design_junction(parse_request(_request_mapping(targets=targets)))

    baseline_assembly_group = next(
        group for group in baseline.assembly_groups if group.assembly_group_id == "assembly-a"
    )
    extended_assembly_group = next(
        group for group in extended.assembly_groups if group.assembly_group_id == "assembly-a"
    )
    assert extended_assembly_group == baseline_assembly_group


def test_universal_recovery_requires_one_pair_per_assembly_group() -> None:
    prefix = "ACGTACGT"
    suffix = "GATTACAA"
    ...
    with pytest.raises(JunctionDesignError, match="universal recovery.*primer pairs differ"):
...
def test_universal_recovery_emits_one_shared_order_pair() -> None:
    ...
    assert {order.order_id for order in recovery_orders} == {
        "assembly-a:universal-recovery-forward",
        "assembly-a:universal-recovery-reverse",
    }
...
def test_universal_recovery_order_ids_cannot_alias_target_specific_orders() -> None:
    ...
    targets = [
        {
            "id": "assembly-a",
            "assembly_group_id": "target-specific-group",
    ...
    result = design_junction(parse_request(_request_mapping(targets=targets)))
    order_ids = [order.order_id for order in result.orders]

    assert len(order_ids) == len(set(order_ids))
    assert "assembly-a:universal-recovery-forward" in order_ids
    assert "assembly-a:universal-recovery-reverse" in order_ids
...
def test_target_specific_ambiguity_uses_binding_sequences_not_extensions() -> None:
    ...
    with pytest.raises(JunctionDesignError, match="Target-specific recovery.*also resolves"):
        design_junction(parse_request(_request_mapping(targets=targets)))
...
def test_recovery_extensions_are_preserved_without_interpreting_downstream_use() -> None:
    ...
    assert recovery.expected_unextended_target == sequence
...
def test_plan_identity_changes_when_seed_changes() -> None:
    ...
    first = design_junction(parse_request(first_mapping))
    second = design_junction(parse_request(second_mapping))
