...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/junction/tests/test_review_projection.py
...
from dnadesign.junction.presentation.review_contract import (
...
def test_review_projection_is_exact_valid_and_structurally_scoped() -> None:
    ...
    assert review.source.algorithm == "dnadesign.junction.string.v1"
    ...
    assert {(check.subject.kind, check.subject.id) for check in review.checks} == {
        ("assembly_group", plan.targets[0].assembly_group_id),
        ("target", plan.targets[0].target_id),
    }
    assert all(
        junction.toehold_span.start
        == next(item for item in plan.assembly_groups[0].junctions if item.junction_id == junction.junction_id).start
...
def test_review_projection_rejects_cross_layer_junction_drift() -> None:
    plan = design_junction(parse_request(_request_mapping()))
    assembly_group = plan.assembly_groups[0]
    junction = assembly_group.junctions[0]
    corrupted_assembly_group = replace(
        assembly_group,
        junctions=(
            replace(junction, barcode="A" * len(junction.barcode)),
            *assembly_group.junctions[1:],
        ),
    )
    corrupted = replace(plan, assembly_groups=(corrupted_assembly_group, *plan.assembly_groups[1:]))

    with pytest.raises(JunctionDesignError, match="inconsistent evidence"):
