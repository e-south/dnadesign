...
from dnadesign.libshuffle.metrics import compute_billboard_metric, compute_pairwise_stats
...
class Subsampler:
    ...
    def run(self):
        ...
        while count < draws:
            idxs = (
                [random.randrange(len(self.seq)) for _ in range(n)]
                if self.cfg.with_replacement
                else random.sample(range(len(self.seq)), n)
            )
