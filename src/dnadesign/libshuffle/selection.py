...
def select_best_subsample(subsamples, cfg, sequences=None):
    ...
    pre = [s for s, m in zip(subsamples, mc) if not np.isnan(m) and m >= thr]
    ...
    survivors = [s for s in survivors if s.get("unique_cluster_count", 0) == cfg.subsample_size]
    ...
    logger.info(f"Selection chosen: {best['subsample_id']} " f"with min_euclidean={best['min_euclidean']:.3e}")
