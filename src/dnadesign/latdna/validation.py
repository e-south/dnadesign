...
def validate_densegen_entry(entry: dict, index: int, source_file: str):
    ...
    """
    required_keys = ["sequence", "sequence_length", "fixed_elements", "meta_tfbs_parts", "meta_tfbs_parts_in_array"]
    ...
    for tf, motif_values in motifs.items():
        ...
        for motif in motif_list:
            if motif in motif_map:
                raise ValueError(
                    f"Duplicate motif detected: '{motif}' is assigned to both '{motif_map[motif]}' and '{tf}'. Please disambiguate."
                )
...
def validate_analysis_entry(entry: dict, index: int, source_file: str):
    ...
    if "evo2_logits_mean_pooled" not in entry:
        raise ValueError(
            f"Entry {index} in file '{source_file}' is missing 'evo2_logits_mean_pooled'. All entries must have been processed by Evo before analysis."
        )
