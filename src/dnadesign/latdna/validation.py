    ...
    for tf, motif_values in motifs.items():
        ...
        for motif in motif_list:
            if motif in motif_map:
                raise ValueError(
                    (
                        f"Duplicate motif detected: '{motif}' is assigned to both "
                        f"'{motif_map[motif]}' and '{tf}'. Please disambiguate."
                    )
                )
...
def validate_analysis_entry(entry: dict, index: int, source_file: str):
    ...
    if "evo2_logits_mean_pooled" not in entry:
        raise ValueError(
            "Results are missing required Evo output key 'logits_mean_pooled'. "
            "All entries must have been processed by Evo before analysis."
        )
