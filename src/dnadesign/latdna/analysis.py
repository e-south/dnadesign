...
from dnadesign.latdna import metrics, utils, validation
...
def run_analysis_pipeline(config: dict):
    ...
    for metric_name in metrics_to_compute:
        ...
        for group_name, vectors in analysis_groups.items():
            ...
            summary_records.append(
                {"metric": metric_name, "group": group_name, "statistic": "mean", "value": float(dist_array.mean())}
            )
            summary_records.append(
                {"metric": metric_name, "group": group_name, "statistic": "std", "value": float(dist_array.std())}
            )

            # Collect the distances in long form
            for d in dist_array:
                rows.append({"metric": metric_name, "group": group_name, "distance": d})
    ...
    fig.suptitle(f"Pairwise intrapopulation distance metrics for ~{total_combos} total pairwise combos", fontsize=12)
