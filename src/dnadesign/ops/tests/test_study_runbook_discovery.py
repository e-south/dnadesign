...
def test_repo_prefixed_runbook_ref_resolves_from_external_study_repository(tmp_path: Path) -> None:
    tool_repo_root = tmp_path / "dnadesign"
    tool_repo_root.mkdir()
    study_repo_root = tmp_path / "research-studies"
    runbook = study_repo_root / "runbooks" / "infer.yaml"
    runbook.parent.mkdir(parents=True)
    runbook.write_text("runbook: {}\n", encoding="utf-8")
    study_dir = study_repo_root / "studies" / "group" / "study"
    _write_study_contract(study_dir, runbook_ref="repo:runbooks/infer.yaml")

    discovered = discover_infer_runbook_paths_for_study(study_dir=study_dir, repo_root=tool_repo_root)
