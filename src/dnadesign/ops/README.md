...
Keep command routing, rendering, and provider loading separate. A study package
owns its status behavior. It registers runtime metadata through
`dnadesign.ops.status_registries` and matching catalog procedure metadata
through `dnadesign.ops.catalog_registries`.
