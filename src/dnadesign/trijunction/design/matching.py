...
from dnadesign.trijunction.design.loci import ToeholdCandidate
from dnadesign.trijunction.design.randomness import StablePrng
from dnadesign.trijunction.design.resources import guard_matching_search
...
def match_barcodes(
    ...
    *,
    ...
    pool_ids = {candidate.pool_id for candidate in ordered_candidates}
    if len(pool_ids) != 1:
        raise ValueError("Toehold/barcode matching requires candidates from one physical pool.")
    ...
    score = winning_key[0]
    barcode_ids = {barcode: f"{pool_id}:barcode-{index:04d}" for index, barcode in enumerate(ordered_barcodes, start=1)}
