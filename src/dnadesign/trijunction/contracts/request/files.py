...
from .model import TriJunctionRequest
...
def load_request(path: str | Path) -> TriJunctionRequest:
    ...
    try:
        ...
        raise TriJunctionConfigError(f"Unable to open TriJunction request: {request_path}") from exc

    try:
        opened = os.fstat(descriptor)
        if not stat.S_ISREG(opened.st_mode):
            raise TriJunctionConfigError(f"TriJunction request is not a regular file: {request_path}")
