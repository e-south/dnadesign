...
Cluster is the exploratory downstream surface for unsupervised clustering, UMAP visualization, and related summaries after one explicit feature definition already exists. It works from one chosen feature matrix in a USR dataset or a CSV/Parquet file, records reusable outputs in a workspace-scoped artifact root, and stays intentionally decoupled from upstream feature generation.

## Documentation

- [Cluster docs](docs/README.md): unified route map for first runs, concepts, references, and cross-tool handoffs.
