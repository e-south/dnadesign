...
def test_workspaces_list_json_reports_builtin_workspace_state(monkeypatch, tmp_path: Path) -> None:
    workspaces_root = tmp_path / "builtin_workspaces"
    ...
    runtime_results_root = tmp_path / "workspaces" / "promoter_clusters_v1" / "outputs" / "cluster"
    runtime_results_root.mkdir(parents=True, exist_ok=True)
    (runtime_results_root / "run.json").write_text(
        "{}\n",
        encoding="utf-8",
    )
    ...
    assert by_id["promoter_clusters_v1"]["results_root"] == str(runtime_results_root.resolve())
