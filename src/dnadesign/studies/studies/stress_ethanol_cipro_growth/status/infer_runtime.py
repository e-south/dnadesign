...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/studies/studies/stress_ethanol_cipro_growth/status/infer_runtime.py
...
class StressEthanolCiproGrowthInferRuntimeDependencies:
...
@dataclass(frozen=True)
class StressEthanolCiproGrowthInferPhaseTarget:
    phase_id: str
    config_label: str
    runtime_label: str
...
@dataclass(frozen=True)
class StressEthanolCiproGrowthInferRuntimeModelSummary:
    label: str
    model_id: str | None
    device: str
...
@dataclass(frozen=True)
class StressEthanolCiproGrowthInferRuntimeResolvedContext:
    ...
    phase_targets: tuple[StressEthanolCiproGrowthInferPhaseTarget, ...]
    phase_targets_by_id: dict[str, StressEthanolCiproGrowthInferPhaseTarget]
    ...
    runtime_model_summaries: tuple[StressEthanolCiproGrowthInferRuntimeModelSummary, ...]
    gpu_required_runtime_labels: tuple[str, ...]


def resolve_stress_ethanol_cipro_growth_infer_runtime_context(
    *,
    ...
    if study_repo_root is None:
        raise ValueError("stress_ethanol_cipro_growth infer-runtime resolution requires a resolved study_repo_root")
    ...
    return StressEthanolCiproGrowthInferRuntimeResolvedContext(
        preferred_model_family=preferred_model_family,
...
def _resolve_runtime_model_summary(
    ...
    *,
    dependencies: StressEthanolCiproGrowthInferRuntimeDependencies,
) -> StressEthanolCiproGrowthInferRuntimeModelSummary:
    ...
    return StressEthanolCiproGrowthInferRuntimeModelSummary(
        label=runtime_label,
...
def _resolve_study_infer_runtime_phase_targets(
    ...
    *,
    study_context: StressEthanolCiproGrowthResolvedContext,
) -> tuple[StressEthanolCiproGrowthInferPhaseTarget, ...]:
    study_repo_root = study_context.study_repo_root
    if study_repo_root is None:
    ...
    targets: list[StressEthanolCiproGrowthInferPhaseTarget] = []
    for runtime_lane in runtime_lane_contracts:
