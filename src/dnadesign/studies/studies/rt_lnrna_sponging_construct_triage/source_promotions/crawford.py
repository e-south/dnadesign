...
LITERATURE_DIR = Path("sources/literature")
...
CRAWFORD_REFERENCE_PATH = (
    LITERATURE_DIR / "Crawford_et_al_2025_retron_ncRNA_ML" / "processed/references/eco1_lnrna_msd_designs.tsv"
)
CRAWFORD_ABUNDANCE_PATH = (
    LITERATURE_DIR / "Crawford_et_al_2025_retron_ncRNA_ML" / "processed/overlays/eco1_ncrna_abundance_observations.tsv"
)
...
def resolve_crawford_promotions(
    *,
    ...
    reference_rows = read_tsv(data_root / CRAWFORD_REFERENCE_PATH)
    abundance_rows = read_tsv(data_root / CRAWFORD_ABUNDANCE_PATH)
    source_row_counts[str(CRAWFORD_REFERENCE_PATH)] = len(reference_rows)
    source_row_counts[str(CRAWFORD_ABUNDANCE_PATH)] = len(abundance_rows)
