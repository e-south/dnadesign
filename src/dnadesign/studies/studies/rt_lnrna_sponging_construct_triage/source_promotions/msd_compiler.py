...
from dnadesign.studies.studies.retron_hairpin_design.catalog.strict_mapping_io import (
...
from dnadesign.studies.studies.retron_hairpin_design.compiler.msd_unit import (
    MsdCompiledUnitV1,
...
def resolve_msd_compiler_promotions(
    *,
    ...
    pool_id = _required_str(spec, "pool_id")
    payload_program_id = _required_str(spec, "payload_program_id")
    max_variant_count = _positive_int(spec.get("max_variant_count"), label="max_variant_count")
    expected_variant_count = _optional_positive_int(spec.get("expected_variant_count"), label="expected_variant_count")
    dedupe_policy = _required_str(spec, "dedupe_policy")
    if dedupe_policy != "fail":
        raise SourcePromotionContractError("rt_lnrna_msd_variant_pool_spec_v1 supports only dedupe_policy=fail.")
    ...
    template_lnrna_sequence = _load_template_lnrna_sequence(
        root=root,
        payload=_mapping(spec.get("template_lnrna"), label="template_lnrna"),
    )
    compiler_inputs = _mapping(spec.get("compiler_inputs"), label="compiler_inputs")
    variant_compiler_payload = _variant_compiler_payload(spec=spec, max_variant_count=max_variant_count)
    template_compiler_payload = {
        "contract": "retron_msd_compiler_spec_v1",
        "schema_version": 1,
        "allow_non_ligatable_s0": True,
        "designs": [_mapping(spec.get("template_msd_design"), label="template_msd_design")],
    ...
    placement = _template_msd_placement(
        ...
        placement=_mapping(spec.get("placement"), label="placement"),
    )
    ...
    if expected_variant_count is not None and len(resolved_variants.catalog.records) != expected_variant_count:
        raise SourcePromotionContractError(
            f"Compiler pool expected {expected_variant_count} variant(s) but emitted "
        ...
        promotions.append(
            SourceConstructSubjectPromotion(
                ...
                overlay_fields=_overlay_fields(
                    ...
                    template_lnrna_ref=_required_str(
                        _mapping(spec["template_lnrna"], label="template_lnrna"),
                        "sequence_ref",
                    ),
                    replacement_span_0=placement["replacement_span_0"],
                    source_refs=_string_list(spec.get("source_refs", []), label="source_refs"),
                ),
            )
        )
...
def _variant_compiler_payload(*, spec: Mapping[str, Any], max_variant_count: int) -> dict[str, Any]:
    compiler_inputs = _mapping(spec.get("compiler_inputs"), label="compiler_inputs")
    has_compiler_spec = "compiler_spec" in spec
    has_design_space = "design_space" in spec
    if has_compiler_spec == has_design_space:
        raise SourcePromotionContractError("Pool spec requires exactly one of compiler_spec or design_space.")
    if has_compiler_spec:
        payload = dict(_mapping(spec.get("compiler_spec"), label="compiler_spec"))
        ...
        if count > max_variant_count:
            raise SourcePromotionContractError(
                f"Compiler pool emits {count} variant(s), which exceeds max_variant_count={max_variant_count}."
            )
        return payload

    design_space = _mapping(spec.get("design_space"), label="design_space")
    payload_ids = _string_list(design_space.get("payload_ids"), label="design_space.payload_ids")
    cap_ids = _string_list(design_space.get("cap_ids"), label="design_space.cap_ids")
    stem_bases = [
        _mapping(item, label="design_space.stem_bases[]")
        for item in _list(design_space.get("stem_bases"), label="design_space.stem_bases")
    ]
    count = len(payload_ids) * len(cap_ids) * len(stem_bases)
    if count > max_variant_count:
        raise SourcePromotionContractError(
            f"Compiler design_space emits {count} variant(s), which exceeds max_variant_count={max_variant_count}."
        )
    construct_prefix = _required_str(design_space, "construct_id_prefix")
    designs: list[dict[str, Any]] = []
    for payload_id, cap_id, stem_base in product(payload_ids, cap_ids, stem_bases):
        stem_id = _required_str(stem_base, "stem_base_id")
        designs.append(
            {
                "construct_id": f"{construct_prefix}__{slug(payload_id)}__{slug(cap_id)}__{slug(stem_id)}",
                "payload_id": payload_id,
                "cap_id": cap_id,
                "left_base": _required_str(stem_base, "left_base"),
                "right_base": _required_str(stem_base, "right_base"),
                "profile_s3s2s1s0": _optional_str(stem_base.get("profile_s3s2s1s0")),
                "source_notes": _optional_str(stem_base.get("source_ref")),
            }
        )
    return {
        "contract": "retron_msd_compiler_spec_v1",
        "schema_version": 1,
        "allow_non_ligatable_s0": bool(spec.get("allow_non_ligatable_s0", False)),
        "designs": designs,
        "payload_sequences": _mapping(
            compiler_inputs.get("payload_sequences"),
            label="compiler_inputs.payload_sequences",
...
def _load_pool_spec(path: Path) -> Mapping[str, Any]:
    if not path.is_file():
    ...
    if payload.get("contract") != "rt_lnrna_msd_variant_pool_spec_v1":
        raise SourcePromotionContractError("MSD compiler pool spec contract must be rt_lnrna_msd_variant_pool_spec_v1.")
    if payload.get("schema_version") != 1:
        raise SourcePromotionContractError("MSD compiler pool spec schema_version must be 1.")
    if payload.get("study_id") != "rt_lnrna_sponging_construct_triage":
        raise SourcePromotionContractError(
            "MSD compiler pool spec study_id must be rt_lnrna_sponging_construct_triage."
        )
    return payload
...
def _string_list(value: Any, *, label: str) -> list[str]:
    ...
    for item in _list(value, label=label):
        ...
        if not text:
            raise SourcePromotionContractError(f"{label} cannot contain blank entries.")
        out.append(text)
    return out
...
def _positive_int(value: Any, *, label: str) -> int:
    if isinstance(value, bool):
        raise SourcePromotionContractError(f"{label} must be a positive integer.")
    try:
        parsed = int(value)
    except (TypeError, ValueError) as exc:
        raise SourcePromotionContractError(f"{label} must be a positive integer.") from exc
    if parsed < 1:
        raise SourcePromotionContractError(f"{label} must be >= 1.")
    return parsed
