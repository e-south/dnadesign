...
_execution = probe_module("tfbs.stage_b.execution")
_assert_selection_budget = _execution._assert_selection_budget
_selected_campaign_rows = _execution._selected_campaign_rows
_write_label_input_for_ids = _execution._write_label_input_for_ids
...
def test_selected_campaign_rows_fail_on_missing_requested_key() -> None:
    ...
    with pytest.raises(ValueError, match="missing requested campaign"):
        _selected_campaign_rows(manifest, ["missing"])
...
def test_assert_selection_budget_fails_for_ordinal_tie_expansion(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="exact-budget selection contract failed"):
        _assert_selection_budget(workdir=tmp_path, round_index=0, selection_k=6, tie_handling="ordinal")
