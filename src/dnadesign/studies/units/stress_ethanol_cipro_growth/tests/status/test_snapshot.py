...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/studies/units/stress_ethanol_cipro_growth/tests/status/test_snapshot.py
...
from dnadesign.studies.units.stress_ethanol_cipro_growth.status.downstream_surfaces import (
...
from dnadesign.studies.units.stress_ethanol_cipro_growth.status.service import (
    STUDY_STATUS_SERVICE,
    StressEthanolCiproGrowthStatusServiceContext,
)
from dnadesign.studies.units.stress_ethanol_cipro_growth.status.snapshot import (
...
def test_stress_ethanol_cipro_status_service_source_is_decomposed_by_responsibility() -> None:
    source_root = _repo_root() / "src" / "dnadesign" / "studies" / "units" / "stress_ethanol_cipro_growth" / "status"
    budgets = {
        "service.py": 320,
...
def test_stress_ethanol_cipro_snapshot_service_keeps_deep_host_and_feature_probes_out_of_record_snapshot(
    ...
    monkeypatch.setattr(
        "dnadesign.studies.units.stress_ethanol_cipro_growth.status.service.build_stress_ethanol_cipro_growth_infer_runtime_dependencies",
