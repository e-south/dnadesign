...
The adapter does not calculate RMF, create observed labels, or mutate
...
uv run python -m dnadesign.studies.units.stress_ethanol_cipro_growth.decision.opal.reader_promoter_evidence \
  ...
  --out-dir src/dnadesign/opal/campaigns/secg_rmf_greedy/inputs/r0 \
  <promoter-evidence-bundle> [<promoter-evidence-bundle> ...]
...
The adapter always emits `campaign_slug: secg_rmf_greedy`, the only
