"""Stage B OPAL config generation for DenseGen TFBS learnability sentinels."""
...
from .seed import (
    ...
    validate_tfbs_stage_b_initial_seed_policy,
)
from .semantics import (
    TFBS_STAGE_B_DEFAULT_TIE_HANDLING,
...
def materialize_tfbs_stage_b_sentinel_configs(config: TfbsStageBConfig) -> TfbsStageBResult:
    ...
    validation = (
        _validate_campaign_configs(campaigns, cfg=cfg) if cfg.validate_configs else _skipped_validation(campaigns)
    )
...
def _normalize_config(config: TfbsStageBConfig) -> TfbsStageBConfig:
...
def _validate_campaign_configs(campaigns: Sequence[Mapping[str, Any]], *, cfg: TfbsStageBConfig) -> dict[str, Any]:
    reports = []
    ok = True
...
def _skipped_validation(campaigns: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    return {
        "status": "SKIPPED",
        "mode": "not_run",
        "campaign_count": int(len(campaigns)),
