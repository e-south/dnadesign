...
@app.command("ingest-msd-regions")
def ingest_msd_regions_command(
    source_genbank: Path = typer.Option(
        ...,
        "--source-genbank",
        exists=True,
        readable=True,
        dir_okay=False,
        help="Monolithic MSD-region GenBank source to decompose into study-owned records.",
    ),
    ...
    try:
        bundle = parse_msd_region_genbank(source_genbank)
