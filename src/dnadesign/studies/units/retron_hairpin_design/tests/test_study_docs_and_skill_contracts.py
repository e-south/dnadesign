...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/studies/units/retron_hairpin_design/tests/test_study_docs_and_skill_contracts.py
...
from dnadesign.studies.units.retron_hairpin_design.catalog.msd_ids import parse_msd_construct_label
...
def test_retron_hairpin_study_is_visible_through_docs_and_agents() -> None:
    ...
    assert "`studies.retron-hairpin-design.status` and" in studies_index
    assert "`studies.retron-hairpin-design.preflight` only for explicit status" in studies_index
    assert "readiness questions" in studies_index
...
def test_retron_hairpin_skill_frontmatter_is_yaml_safe_and_discovery_scoped() -> None:
    ...
    assert metadata["version"] == "0.7.19"
...
def test_retron_hairpin_study_record_and_skill_keep_boundary_language_explicit() -> None:
    ...
    status_note = _read("docs/studies/retron_hairpin_design/record/status.md")
    ...
    assert "Current phase:" not in status_note
    assert "snapback_released_solve" not in status_note
    ...
    assert "Open `../operations/runtime/command-groups/pipeline.yaml` only when the task needs" in routes
    ...
    assert "src/dnadesign/studies/units/retron_hairpin_design/compiler/references.py" in study_surfaces
    assert "src/dnadesign/studies/units/retron_hairpin_design/compiler/catalog_bundle.py" in study_surfaces
    assert "src/dnadesign/studies/units/retron_hairpin_design/compiler/materialization.py" in study_surfaces
    assert "src/dnadesign/studies/units/retron_hairpin_design/compiler/exceptions.py" in study_surfaces
    assert "src/dnadesign/studies/units/retron_hairpin_design/interfaces/cli/app.py" in study_surfaces
    assert "src/dnadesign/studies/units/retron_hairpin_design/catalog/compiler_spec_io.py" in study_surfaces
...
def test_retron_hairpin_workbench_keeps_root_bounded_by_record_lanes() -> None:
    ...
    visible_root_dirs = {
        ...
        if path.is_dir() and not path.name.startswith(".") and path.name not in generated_dirs
    }
