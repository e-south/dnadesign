...
def test_conservation_materializer_writes_long_form_profile(tmp_path: Path) -> None:
    ...
    assert metadata[b"profile_ids"] == b'["ec86_clade9_conservation_v1", "ec86_iia3_cluster42_1_conservation_v1"]'

    rows = table.to_pylist()
    position_3 = next(
        row for row in rows if row["profile_id"] == "ec86_clade9_conservation_v1" and row["canonical_position"] == 3
    )
    ...
    position_4 = next(
        row for row in rows if row["profile_id"] == "ec86_clade9_conservation_v1" and row["canonical_position"] == 4
    )
    assert position_4["wt_is_plurality"] is False
    assert position_4["passes_conservation_mask"] is False

    position_1 = next(
        row for row in rows if row["profile_id"] == "ec86_clade9_conservation_v1" and row["canonical_position"] == 1
    )
...
def test_phase1_rejects_conservation_profile_source_hash_mismatch(tmp_path: Path) -> None:
    ...
    rows = table.to_pylist()
    for row in rows:
        if row["profile_id"] == "ec86_clade9_conservation_v1":
