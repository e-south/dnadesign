...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/studies/units/eco1_rt_repack/tests/materialization/proteinmpnn_sample_ingest/test_materialization.py

ProteinMPNN sample-ingest materialization tests for Eco1 RT repack.
...
def test_proteinmpnn_sample_ingest_writes_phase2_sample_table(tmp_path: Path) -> None:
    ...
    rows = pq.read_table(result.sample_table_path).to_pylist()
    assert result.backend_run_manifest_path.exists()
    assert len(rows) == 96
    assert {row["seed"] for row in rows} == {101, 202, 303}
    ...
    phase2 = validate_checked_in_contracts(
        repo_root=repo_root(),
        phase="phase2_real_backend_ingest",
        output_root=tmp_path,
    )
...
def _fake_runner(
    *,
    ...
    for seed in manifest["seed_set"]:
        ...
        records = [
            (
                f">chain_a_backbone, score=1.0000, global_score=2.0000, fixed_chains=[], designed_chains=['A'], "
                f"model_name=v_48_020, git_hash=fake, seed={seed}"
            ),
            wt_sequence,
        ]
