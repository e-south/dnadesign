...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/studies/units/eco1_rt_repack/tests/materialization/candidate_table/test_materialization.py

Candidate-table materialization tests for Eco1 RT repack.
...
def test_candidate_table_materializes_from_current_sample_table(tmp_path: Path) -> None:
    ...
    assert len(rows) <= 96
    assert rows
    assert all(row["protected_mutation_count"] == 0 for row in rows)
...
def _fake_runner(
    *,
    ...
    for seed in manifest["seed_set"]:
        ...
        fasta.write_text("\n".join(records) + "\n", encoding="utf-8")
        rows.append({"seed": seed, "output_dir": str(seed_dir), "returncode": 0, "stdout": "", "stderr": ""})
