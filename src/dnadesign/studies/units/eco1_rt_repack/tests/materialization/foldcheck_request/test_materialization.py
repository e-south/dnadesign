...
from dnadesign.studies.units.eco1_rt_repack.operations.contracts.foldcheck.request import (
...
def test_foldcheck_request_validator_rejects_fasta_sequence_hash_drift(tmp_path: Path) -> None:
    write_minimal_foldcheck_inputs(tmp_path)
    result = materialize_foldcheck_request(repo_root=Path.cwd(), output_root=tmp_path)
    fasta_lines = result.input_fasta_path.read_text(encoding="utf-8").splitlines()
    sequence_line_index = next(index for index, line in enumerate(fasta_lines) if line and not line.startswith(">"))
    fasta_lines[sequence_line_index] = "C" + fasta_lines[sequence_line_index][1:]
    result.input_fasta_path.write_text("\n".join(fasta_lines) + "\n", encoding="utf-8")

    issues = validate_foldcheck_request_content(result.request_manifest_path, output_root=tmp_path)

    assert "eco1_rt.foldcheck_request.sequence_hash_mismatch" in {issue.check_id for issue in issues}
