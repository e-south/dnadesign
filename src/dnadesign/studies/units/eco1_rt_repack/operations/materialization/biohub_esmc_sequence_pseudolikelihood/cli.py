...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/studies/units/eco1_rt_repack/operations/materialization/biohub_esmc_sequence_pseudolikelihood/cli.py

CLI for Eco1 Biohub ESMC sequence pseudo-likelihood scoring.
...
from .pipeline import materialize_biohub_esmc_sequence_pseudolikelihood


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Materialize Eco1 Biohub ESMC leave-one-out sequence pseudo-likelihood artifacts."
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Specific candidate sequence id to score; WT is always retained when any candidate id is supplied.",
    )
    parser.add_argument(
        "--positions",
    ...
    print(f"position_pll: {result.position_pll_path}")
    print(f"sequence_pll: {result.sequence_pll_path}")
