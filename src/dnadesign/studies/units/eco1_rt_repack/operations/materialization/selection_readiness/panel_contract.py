...
def variant_id_for_policy(*, policy_id: str, within_group_rank: int) -> str:
    """Return the concise immutable alias assigned when the selected panel is frozen."""

    allocation = allocation_for_policy(policy_id)
    if not 1 <= within_group_rank <= allocation.selected_count:
        raise ValueError(
            f"Variant rank for {policy_id!r} must be between 1 and {allocation.selected_count}, "
            f"received {within_group_rank}."
        )
    return f"{VARIANT_ID_PREFIX}-G{GENERATION_POLICY_VERSION}-{allocation.variant_code}{within_group_rank:02d}"
