...
def test_reader_spop_composite_smoke_renders_missing_condition_cells(tmp_path: Path) -> None:
    matrix = ReaderSpopConditionMatrix(
        rows=(
            _row("retron26", "0 nm aTc; 0 uM IPTG", "baseline", 0.0, 0.0, 100.0),
            _row("retron26", "20 nm aTc; 0 uM IPTG", "positive_control", 20.0, 0.0, 500.0),
            _row("retron26", "0 nm aTc; 500 uM IPTG", "iptg_dose", 0.0, 500.0, 460.0),
            _row("retron195", "0 nm aTc; 0 uM IPTG", "baseline", 0.0, 0.0, 100.0),
            _row("retron195", "20 nm aTc; 0 uM IPTG", "positive_control", 20.0, 0.0, 500.0),
        ),
        condition_columns=(
            ReaderSpopConditionColumn(
            ...
            ReaderSpopConditionColumn(
                condition_key="20 nm aTc; 0 uM IPTG",
                condition_role="positive_control",
                atc_nM=20.0,
                iptg_uM=0.0,
            ),
            ReaderSpopConditionColumn(
                condition_key="0 nm aTc; 500 uM IPTG",
...
def _row(
    ...
    normalized = 0.0 if condition_role == "baseline" else 1.0 if condition_role == "positive_control" else 0.9
