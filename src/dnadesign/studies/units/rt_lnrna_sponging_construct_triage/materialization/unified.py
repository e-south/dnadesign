...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/studies/units/rt_lnrna_sponging_construct_triage/materialization/unified.py
...
from .subject_binding_adapter import _adapt_registered_subject_bindings
...
def _select_unified_construct_subjects(
    *,
    ...
    subject_binding_subject_ids: tuple[str, ...] | None,
    allow_partial_byte_resolution: bool,
    ...
    subject_binding_count, blocked_subject_bindings = _append_subject_binding_subjects(
        context=context,
        rows=rows,
        expected_sequences=expected_sequences,
        run_plans=run_plans,
        subject_ids=subject_binding_subject_ids,
    )
    blocked_subject_binding_count = len(blocked_subject_bindings)
    requested_subject_binding_count = subject_binding_count + blocked_subject_binding_count
    if blocked_subject_bindings and not allow_partial_byte_resolution:
        blocked_ids = ", ".join(block.subject_id for block in blocked_subject_bindings)
        raise MaterializationContractError(
            "Subject-binding materialization is incomplete because provider publications omit RT CDS bytes; "
            "set allow_partial_byte_resolution=True to materialize only resolved subjects. "
            f"blocked_subject_ids=[{blocked_ids}]"
        )
    crawford_count = 0
    khan_count = 0
    ...
    return _UnifiedConstructSubjectSelection(
        ...
        subject_binding_requested_subject_count=requested_subject_binding_count,
        subject_binding_resolved_subject_count=subject_binding_count,
        subject_binding_blocked_subject_count=blocked_subject_binding_count,
        subject_binding_resolution_complete=not blocked_subject_bindings,
        blocked_subject_bindings=blocked_subject_bindings,
...
def _append_subject_binding_subjects(
    *,
    context: _MaterializationContext,
    rows: list[dict[str, object]],
    expected_sequences: dict[str, str],
...
) -> tuple[int, tuple[SubjectBindingByteBlock, ...]]:
    projection = _adapt_registered_subject_bindings(context=context, subject_ids=subject_ids)
    _extend_construct_subject_rows(rows, projection.rows)
    expected_sequences.update(projection.expected_sequences)
    for group_index, (window_offset_bp, subject_ids) in enumerate(
        projection.subject_ids_by_window_offset.items(), start=1
    ):
        group_name = f"subject_bindings_offset_{group_index:02d}"
        run_plans.append(
            _ConstructViewRunPlan(
    ...
    return len(projection.rows), projection.blocked_subjects
