"""
Promoter-study binding and snapshot helpers for the LatentDNA seam.
"""
...
DEFAULT_LATENTDNA_DOC = "src/dnadesign/latentdna/docs/workflows/promoter-study-representation-comparison.md"
...
def latentdna_binding_path(study_context: PromoterStudyResolvedContext) -> Path | None:
...
def load_latentdna_binding(study_context: PromoterStudyResolvedContext) -> tuple[Path | None, dict[str, object] | None]:
...
def latentdna_doc_path(study_context: PromoterStudyResolvedContext) -> str:
    latentdna_config = study_context.study_pipeline.get("latentdna")
    if isinstance(latentdna_config, Mapping):
