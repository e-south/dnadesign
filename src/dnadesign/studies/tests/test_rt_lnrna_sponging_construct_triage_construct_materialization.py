...
def test_rt_lnrna_controls_materialize_real_1600bp_construct_context_views(tmp_path: Path) -> None:
    ...
    assert {result.records_total for result in report.run_results} == {2, 4}
    ...
    for candidate_id, expected in expected_spans.items():
        ...
        assert forward["sequence"][:lnrna_start] == report.template_context_sequence[:lnrna_start]
        assert forward["sequence"][lnrna_end:rt_start] == report.template_context_sequence[359:524]
        suffix_length = 1600 - rt_end
        assert forward["sequence"][rt_end:] == report.template_context_sequence[1487 : 1487 + suffix_length]
