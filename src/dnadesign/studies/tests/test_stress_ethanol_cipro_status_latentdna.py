"""
Stress ethanol cipro status LatentDNA seam tests.
"""
...
from dnadesign.studies.studies.stress_ethanol_cipro_growth.status.analysis_surfaces import (
    inspect_stress_ethanol_cipro_growth_exploratory_analysis,
)
from dnadesign.studies.studies.stress_ethanol_cipro_growth.status.latentdna_contract import (
    validate_binding,
    validate_workspace_snapshot,
)
from dnadesign.studies.studies.stress_ethanol_cipro_growth.status.latentdna_readiness import (
    inspect_stress_ethanol_cipro_growth_latentdna_readiness,
)
from dnadesign.studies.tests.test_stress_ethanol_cipro_status_snapshot import _make_study_context

LATENTDNA_DOC = "src/dnadesign/latentdna/docs/workflows/stress-ethanol-cipro-representation-comparison.md"
...
def test_inspect_stress_ethanol_cipro_growth_latentdna_readiness_uses_binding_and_snapshot_contract(
    tmp_path: Path,
) -> None:
...
def test_stress_ethanol_cipro_latentdna_binding_accepts_generic_source_dataset_keys() -> None:
...
def test_stress_ethanol_cipro_latentdna_snapshot_validation_requires_study_binding(tmp_path: Path) -> None:
    snapshot_path = _write_latentdna_snapshot_fixture(tmp_path)
...
def test_inspect_stress_ethanol_cipro_growth_latentdna_readiness_rejects_snapshot_schema_mismatch(
    tmp_path: Path,
) -> None:
    binding_path = _write_latentdna_binding_fixture(tmp_path)
    snapshot_path = _write_latentdna_snapshot_fixture(tmp_path)
...
def test_inspect_stress_ethanol_cipro_growth_latentdna_readiness_rejects_structurally_invalid_snapshot(
    tmp_path: Path,
) -> None:
    binding_path = _write_latentdna_binding_fixture(tmp_path)
    snapshot_path = _write_latentdna_snapshot_fixture(tmp_path)
...
def test_inspect_stress_ethanol_cipro_growth_latentdna_readiness_rejects_malformed_binding(tmp_path: Path) -> None:
    binding_path = _write_latentdna_binding_fixture(tmp_path)
...
def test_inspect_stress_ethanol_cipro_growth_exploratory_analysis_reports_snapshot_backed_latentdna_surface(
    tmp_path: Path,
) -> None:
    binding_path = _write_latentdna_binding_fixture(tmp_path)
    _write_latentdna_snapshot_fixture(tmp_path)
    ...
    latentdna_state = inspect_stress_ethanol_cipro_growth_latentdna_readiness(study_context=study_context)

    surfaces = inspect_stress_ethanol_cipro_growth_exploratory_analysis(
...
def test_stress_ethanol_cipro_latentdna_status_modules_do_not_import_latentdna_internals() -> None:
    repo_root = Path(__file__).resolve().parents[4]
    for relative_path in [
        "src/dnadesign/studies/studies/stress_ethanol_cipro_growth/status/latentdna_readiness.py",
