...
### candidate_decision_frontier | Candidate decision frontier
...
**Definition.** Point size is effective rank,
...
where the \(\lambda_i\) values are the positive explained-variance ratios of the centered representation spectrum. In plain language, a larger point means variance is spread across more directions rather than collapsing into one dominant component.
...
**Decision use.** Read this as a synthesis of the healthy-candidate ladder rather than a replacement for it. The plot is strongest when it confirms what the gate, Sigma-35 audit, and direct context comparison already say independently. A candidate is more convincing when it sits farther right and higher up while also keeping a reasonably large effective-rank point, because that combination suggests preserved structure without obvious collapse.
