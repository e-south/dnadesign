...
def test_snapback_design_emits_public_jobs_that_render_pngs(tmp_path: Path) -> None:
    spec_path = _write_workspace(tmp_path)
    run_dir, report = run_snapback_design(spec_path)

    assert report.status == "satisfied"
    for job_name, render_name in (
        ("pre_nick_duplex.job.yaml", "pre_nick_duplex.png"),
        ("post_nick_exposed.job.yaml", "post_nick_exposed.png"),
        ("post_nick_foldback.job.yaml", "post_nick_foldback.png"),
    ):
        job_path = run_dir / "baserender_jobs" / job_name
        render_path = run_dir / "renders" / render_name
