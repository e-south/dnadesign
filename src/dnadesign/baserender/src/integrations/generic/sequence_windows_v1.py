...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/baserender/src/integrations/generic/sequence_windows_v1.py
...
_DISPLAY_KEYS = {"title", "overlay_text", "tag_labels", "trajectory_panel"}
...
class SequenceWindowsV1Adapter:
    ...
    def _parse_display(self, raw: Any, *, tag_labels: Mapping[str, str]) -> Display:
        ...
        if overlay_text == "":
            overlay_text = None
        trajectory_panel_raw = data.get("trajectory_panel")
        ...
        return Display(overlay_text=overlay_text, tag_labels=merged_labels, trajectory_panel=trajectory_panel)
