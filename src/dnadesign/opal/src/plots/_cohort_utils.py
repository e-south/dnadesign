...
def selected_mask(values: pd.Series, *, allow_null_false: bool = False) -> pd.Series:
    if values.isna().any():
        if not allow_null_false:
            raise ValueError("view__is_selected contains null values.")
    ...
    if not pd.api.types.is_bool_dtype(values):
        ...
        if not bad.empty:
            ...
            raise ValueError(f"view__is_selected must be boolean; got {preview}")
    return values.astype(bool)


def positive_ranks(values: pd.Series, *, column: str = "view__rank_competition") -> pd.Series:
