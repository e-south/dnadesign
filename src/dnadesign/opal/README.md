...
If you only need exploratory structure, UMAPs, or cluster-level summaries over that chosen `infer__...` column, use [cluster exploratory clustering workflow](../cluster/docs/workflows/exploratory-clustering.md) instead.
...
2. [Run a USR dataset with infer-derived X -> OPAL workflow](./docs/workflows/usr-infer-x-active-learning.md) when the feature matrix already exists in USR.
3. [Run an RF + SFXI + top_n workflow](./docs/workflows/rf-sfxi-topn.md) for the baseline operational path.
4. [Run a GP + SFXI + top_n workflow](./docs/workflows/gp-sfxi-topn.md) for GP-driven top_n selection.
5. [Run a GP + SFXI + expected_improvement workflow](./docs/workflows/gp-sfxi-ei.md) for EI-based candidate selection.
6. [Use the CLI reference for command contracts and flags](./docs/reference/cli.md) before scripting or automation.
7. [Read Gaussian Process behavior and math](./docs/plugins/model-gaussian-process.md) to inspect GP assumptions and outputs.
8. [Read Expected Improvement behavior and math](./docs/plugins/selection-expected-improvement.md) to understand EI scoring and selection.
9. [Read SFXI behavior and math](./docs/plugins/objective-sfxi.md) to interpret objective-channel semantics.
