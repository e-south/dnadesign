...
from dnadesign.nmf.persistence import annotate_sequences_with_nmf, export_feature_names, export_row_ids
...
def main():
    ...
    plot_motif_occurrence_heatmap(
        X, str(motif_occurrence_heatmap_path), max_occ=nmf_config.get("max_motif_occupancy", 5), transpose=True
    )
    ...
    plot_program_association_stacked_barplot(
        H_best, feature_names, str(program_assoc_stacked_path), motif2tf_map=motif2tf_map, normalize=True
    )
    ...
    if best_k is not None:
        ...
        try:
            ...
            if not W_file.exists() or not H_file.exists():
                ...
                plot_sequence_program_composition(W, str(plots_dir / "sequence_program_composition.png"))
