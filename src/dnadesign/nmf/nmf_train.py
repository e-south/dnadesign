...
def train_nmf(X: np.ndarray, config: dict) -> dict:
    ...
    for k in k_range:
        ...
        cache_exists = (os.path.exists(os.path.join(k_dir, "W.csv")) and 
                        os.path.exists(os.path.join(k_dir, "H.csv")) and
                        os.path.exists(os.path.join(k_dir, "metrics.yaml")))
        if cache_exists:
        ...
        for i in range(n_init):
            try:
                model = SklearnNMF(n_components=k, init=init_method, solver=solver,
                                   max_iter=max_iter, random_state=i, beta_loss=loss_metric, tol=1e-4,
