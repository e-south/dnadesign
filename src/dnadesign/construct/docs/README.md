...
through stable USR datasets and `construct__*` lineage. It also has a
workspace-less `compose` route for declared linear ssDNA segment specs that
should become local artifact bundles. Use this page to choose the next
Construct task.
...
- [Compose a declared linear ssDNA product](reference/linear-ssdna-composition.md): validate a segment-spec YAML, emit a producer-owned bundle, and optionally publish the two-panel composition review.
