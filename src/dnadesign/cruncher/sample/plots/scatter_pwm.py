...
def plot_scatter_pwm(sample_dir: Path, pwms: dict[str, PWM], cfg: CruncherConfig) -> None:
    ...
    sc = ax.scatter(
        df_sub[f"score_{x_tf}"], df_sub[f"score_{y_tf}"], c=df_sub["iter"], cmap=cmap, alpha=0.6, s=30, linewidth=0
    )
    ...
    for cx, cy, tf in consensus_points:
        ax.scatter(cx, cy, marker="*", s=200, edgecolor="black", facecolor="yellow", label=f"consensus_{tf}")
