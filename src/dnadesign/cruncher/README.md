...
- **payload-centric YIU workflows:** validate strict `.yiu.yaml` payload specs, normalize `user_sequence` or `sample_hit` inputs, exhaustively optimize a 4-nt junction and per-position mismatches with optional PWM context, and publish one bundle-local contract surface under `outputs/<workflow>/` plus a mirrored operator PDF when configured.
...
9. [YIU workflow](docs/guides/yiu_workflow.md): route the strict YIU bundle, render, and inspection flow.
