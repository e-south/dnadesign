...
### Step-by-Step Commands
    ...
    set -euo pipefail
    cd src/dnadesign/cruncher/workspaces/demo_monotypic_soxs
    ...
    # fetch sites --source providers for soxS before discovery.
    cruncher fetch sites --source regulondb --tf soxS --update -c "$CONFIG"
    # Discover motifs from fetched site evidence. This step merges all fetched site sets across sources and runs MEME OOPS into the workspace-specific source id.
    cruncher discover motifs --set 1 --tool meme --meme-mod oops --source-id demo_soxs_multiplicity_meme_oops -c "$CONFIG"
    ...
    cruncher catalog logos --source demo_soxs_multiplicity_meme_oops --set 1 -c "$CONFIG"
    # Validate and render the sample-backed YIU payload bundle under outputs/plots, then inspect that bundle.
    cruncher yiu validate --spec configs/yiu/soxs_monotypic_hit.yiu.yaml
    cruncher yiu render --spec configs/yiu/soxs_monotypic_hit.yiu.yaml --force-overwrite --emit-renders
    cruncher yiu show --bundle outputs/plots/yiu__soxs_monotypic_hit
...
### Optional output checks
    ...
    find outputs/plots/yiu__soxs_monotypic_hit -maxdepth 1 -type f | sort
