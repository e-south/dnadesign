...
### Step-by-Step Commands
    ...
    set -euo pipefail
    cd src/dnadesign/cruncher/workspaces/demo_monotypic_lexa
    ...
    # Merge local MEME-derived LexA sites with RegulonDB evidence before discovery.
    cruncher fetch sites --source demo_local_meme --tf lexA --update -c "$CONFIG"
    cruncher fetch sites --source regulondb --tf lexA --update -c "$CONFIG"
    ...
    cruncher discover motifs --set 1 --tool meme --meme-mod oops --source-id demo_lexa_multiplicity_meme_oops -c "$CONFIG"
    ...
    cruncher catalog logos --source demo_lexa_multiplicity_meme_oops --set 1 -c "$CONFIG"
    # Validate and render the sample-backed YIU payload bundle under outputs/plots, then inspect that bundle.
    cruncher yiu validate --spec configs/yiu/lexa_monotypic_hit.yiu.yaml
    cruncher yiu render --spec configs/yiu/lexa_monotypic_hit.yiu.yaml --force-overwrite --emit-renders
    cruncher yiu show --bundle outputs/plots/yiu__lexa_monotypic_hit
...
### Optional output checks
    ...
    find outputs/plots/yiu__lexa_monotypic_hit -maxdepth 1 -type f | sort
