...
### Step-by-Step Commands
    ...
    set -euo pipefail
    cd src/dnadesign/cruncher/workspaces/demo_monotypic_soxr
    ...
    # Merge local MEME-derived SoxR sites with RegulonDB evidence before discovery.
    cruncher fetch sites --source demo_local_meme --tf soxR --update -c "$CONFIG"
    cruncher fetch sites --source regulondb --tf soxR --update -c "$CONFIG"
    ...
    cruncher discover motifs --set 1 --tool meme --meme-mod oops --source-id demo_soxr_multiplicity_meme_oops -c "$CONFIG"
    ...
    cruncher catalog logos --source demo_soxr_multiplicity_meme_oops --set 1 -c "$CONFIG"
    # Validate and render the sample-backed YIU payload bundle under outputs/plots, then inspect that bundle.
    cruncher yiu validate --spec configs/yiu/soxr_monotypic_hit.yiu.yaml
    cruncher yiu render --spec configs/yiu/soxr_monotypic_hit.yiu.yaml --force-overwrite --emit-renders
    cruncher yiu show --bundle outputs/plots/yiu__soxr_monotypic_hit
...
### Optional output checks
    ...
    find outputs/plots/yiu__soxr_monotypic_hit -maxdepth 1 -type f | sort
