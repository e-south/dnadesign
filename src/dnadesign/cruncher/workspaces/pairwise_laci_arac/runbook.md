## pairwise_laci_arac Runbook

**Workspace Path**
- src/dnadesign/cruncher/workspaces/pairwise_laci_arac/

**Regulators**
- [lacI, araC]

**Purpose**
- Run the full Cruncher flow for this regulator set: fetch sites, discover motifs, lock inputs, sample sequences, analyze outputs, and export handoff artifacts (including DenseGen PWM export).
...
### Step-by-Step Commands
    ...
    set -euo pipefail
    cd src/dnadesign/cruncher/workspaces/pairwise_laci_arac
    ...
    cruncher fetch sites --source demo_local_meme --tf lacI --update -c "$CONFIG"
    cruncher fetch sites --source regulondb --tf lacI --tf araC --update -c "$CONFIG"
    ...
    cruncher discover motifs --set 1 --tool meme --meme-mod oops --source-id pairwise_laci_arac_merged_meme_oops -c "$CONFIG"
    ...
    cruncher catalog logos --source pairwise_laci_arac_merged_meme_oops --set 1 -c "$CONFIG"
    # Export DenseGen motif artifacts for downstream handoff workspaces.
...
### Optional output checks
    ...
    ls -1 ../../../densegen/workspaces/study_constitutive_sigma_panel/inputs/motif_artifacts
