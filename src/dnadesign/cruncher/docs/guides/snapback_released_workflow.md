...
This lane exists because the final active product may be smaller than the recognition sites required to build it. The default operational evaluation target is still the rebased exposed bottom strand. The reporting and triptych surfaces stay strand-explicit so broader retained-active audits can still be rendered when operators deliberately opt in.
...
- `released-target-search` searches nickase plus release-enzyme combinations for exact or near exposed-bottom hits by default
- `released-solve` materializes ranked exact or near exposed-bottom hits by default and can emit one origin-anchored plot per hit
- `released-show` revalidates one `released-design` explicit bundle and fails fast on drift
...
- optional retained-active audits stay opt-in via `--allow-top-active-routes` plus `--allow-precut-footprint-outside-active-product`
...
- **final-geometry space** is the active post-release strand after nick then release, rebased so the nick boundary remains the origin coordinate
...
# Optional broader retained-active audit when you need retained-top routes and
# full pre-cut nickase-footprint constructability.
cruncher snapback released-target-search \
  --workspace-root . \
  --nick-preset neb_nicking_v1 \
  --nick-additional-preset thermo_nicking_v1 \
...
3. project the active strand plus any surviving partner fragment
4. evaluate the final geometry on that active product
...
`--allow-precut-footprint-outside-active-product` is enabled, the search
instead preserves the full vendor footprint and asks whether the precursor is
constructable even when some pre-cut site bases sit outside the final active
product.
...
`--emit-renders` is enabled. The solve plot keeps the nick at the rebased origin
boundary in the released and foldback panels, keeps precursor and post-release
rows on physical top/bottom lanes, and preserves the physical nicked strand
when the foldback panel rebases the active strand at origin. Surviving upstream
context left of the nick stays visible in the released and foldback panels, so
the rebased origin can still be read against the real retained duplex context.
The solve hit table reports the route-policy fields
(`final_geometry_source`, `route_family`, `active_strand`,
`retained_partner_strand`, `physical_nicked_strand`) plus the generic
`active_product_*` metrics and retained-partner fragment length.
...
requires the origin-anchored stem/foldback return on the active strand
...
Use `--allow-demo-hits` when you intentionally want demo-only entries to flow
