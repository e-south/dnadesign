...
> **Workflow families:** Cruncher registers five peer command families.
>
> - `fetch|lock|parse|sample|analyze|export` cover fixed-length sequence optimization.
> - `cassette init-workspace|validate|design|solve|show` cover generic cassette planning and bounded search.
> - `yiu init-workspace|validate|render|show` cover payload-centric YIU workflows.
> - `study run|summarize|show` orchestrate aggregate sweeps over explicit source artifacts.
> - `portfolio run|show` aggregate study-ready workspaces into handoff tables and plots.
>
> Choose the family whose workspace contract matches the intended output. Families do not silently fall back to `sample` or reuse another family's artifacts.
