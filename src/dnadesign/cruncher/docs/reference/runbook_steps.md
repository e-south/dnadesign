...
| `demo_snapback` | `snapback_validate` | Validate the checked-in explicit snapback demo spec. | `cruncher snapback validate --spec configs/snapback/demo_teto_bpu10i_cap.snapback.yaml` |
| `demo_snapback` | `snapback_design` | Materialize the checked-in explicit snapback demo bundle. | `cruncher snapback design --spec configs/snapback/demo_teto_bpu10i_cap.snapback.yaml --force-overwrite` |
| `demo_snapback` | `snapback_show_design` | Inspect the explicit snapback bundle and integrity checks. | `cruncher snapback show --run outputs/design` |
| `demo_snapback` | `snapback_released_design` | Materialize the checked-in released-product precursor demo bundle. | `cruncher snapback released-design --spec configs/snapback/demo_released_origin_033.released.snapback.yaml --force-overwrite` |
| `demo_snapback` | `snapback_released_show` | Inspect the released-product bundle and projection integrity checks. | `cruncher snapback released-show --run outputs/released_design` |
| `demo_snapback` | `snapback_released_target_search` | Search paired nickase plus release-enzyme placements against the checked-in demo overlays. | `cruncher snapback released-target-search --workspace-root . --nick-additional-path inputs/nickases/local.nickases.yaml --release-additional-path inputs/release_enzymes/local.release.yaml --nick-boundary 0 --paired-bp 3 --cap-nt 3 --json` |
| `demo_snapback` | `snapback_solve` | Run the broader catalog-scan solve demo. | `cruncher snapback solve --spec configs/snapback/demo_teto_catalog_scan.snapback.solve.yaml --force-overwrite` |
| `demo_snapback` | `snapback_show_solve` | Inspect the solve bundle, frontier, and materialized hit paths. | `cruncher snapback show --run outputs/solve` |
