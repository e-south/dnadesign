...
The bundled config uses targeted insertion proposals plus adaptive move weights/proposal sizing with a six-chain Gibbs annealing optimizer (`optimizer.kind=gibbs_anneal`, `optimizer.chains=6`, `cooling.kind=linear`, `cooling.beta_end=3.4`) and a fixed softmin schedule (`beta_end=6`) for a less jumpy optimization trajectory.
