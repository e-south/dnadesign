"""Target inspection helpers for config-defined TF sets."""
...
def target_statuses(
    *,
    ...
    for set_index, tf in list_targets(cfg):
        ...
        if entry.has_sites and entry.site_total > entry.site_count:
            missing = entry.site_total - entry.site_count
            seq_msg = (
                f"{missing} of {entry.site_total} sites lack sequences. "
                "Configure ingest.genome_source=ncbi or pass --genome-fasta to hydrate sequences."
            )
            if message:
                message = f"{message} {seq_msg}"
            else:
                message = seq_msg
        if status in {"ready", "warning"}:
            ...
            if effective_pwm_source == "sites" and entry.has_sites:
                sites_path = catalog_root / "normalized" / "sites" / entry.source / f"{entry.motif_id}.jsonl"
                if not sites_path.exists():
                    status = "missing-sites-file"
                    message = f"Sites file missing: {sites_path}"
                if entry.site_length_min is not None and entry.site_length_max is not None:
                    if entry.site_length_min != entry.site_length_max:
                        length = resolve_window_length(
                        ...
                        if length is None:
                            status = "needs-window"
                            message = (
                                "Site lengths vary; set motif_store.site_window_lengths for this TF or dataset "
                                "before building PWMs."
                            )
        statuses.append(
            TargetStatus(
                set_index=set_index,
