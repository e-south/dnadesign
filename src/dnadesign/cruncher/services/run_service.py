...
def run_index_path(config_path: Path) -> Path:
    return config_path.parent / ".cruncher" / "run_index.json"


def load_run_index(config_path: Path) -> dict[str, dict]:
...
def save_run_index(config_path: Path, payload: dict[str, dict]) -> Path:
    path = run_index_path(config_path)
...
def update_run_index_from_manifest(config_path: Path, run_dir: Path, manifest: dict) -> dict:
    updates = _updates_from_manifest(manifest)
    updates.setdefault("run_dir_guess", str(run_dir.resolve()))
    return upsert_run_index(config_path, run_dir.name, updates)


def update_run_index_from_status(config_path: Path, run_dir: Path, status_payload: dict) -> dict:
...
def rebuild_run_index(cfg: CruncherConfig, config_path: Path) -> Path:
    ...
    return save_run_index(config_path, index)
