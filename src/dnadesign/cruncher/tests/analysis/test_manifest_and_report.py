...
Regression tests for manifest and report Cruncher analysis.
...
def test_report_payload_paths_use_structured_output_and_plots_schema(tmp_path: Path) -> None:
    ...
    analysis_plot_path(analysis_dir, "elite_score_space_context", "png").parent.mkdir(parents=True, exist_ok=True)
    analysis_plot_path(analysis_dir, "elite_score_space_context", "png").write_text("png")
    ...
    assert pointers["start_here_plot"] == "plots/elite_score_space_context.png"
    assert pointers["trajectory_plot"] == "plots/elite_score_space_context.png"
