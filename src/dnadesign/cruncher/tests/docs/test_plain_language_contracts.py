...
def _plain_text_only(markdown: str) -> str:
    without_fences = FENCED_CODE.sub(" ", markdown)
    return INLINE_CODE.sub(" ", without_fences)
...
def test_cruncher_docs_and_workspaces_avoid_canonical_wording() -> None:
    ...
    for path in markdown_files:
        content = _plain_text_only(path.read_text(encoding="utf-8"))
