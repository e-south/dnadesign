...
def test_s3_s2_s1_s0_profile_matches_reference_controls() -> None:
    assert classify_pair_profile("GCCC", "TGTC").profile_s3s2s1s0 == "MXMX"
    assert classify_pair_profile("GTTC", "GAGC").profile_s3s2s1s0 == "MXMM"


def test_profile_reports_pair_classes_in_s3_s2_s1_s0_order() -> None:
    profile = classify_pair_profile("GCCC", "TGTC")
    ...
    assert profile.worst_hard_mismatch_tier == 3
    assert profile.hard_mismatch_tier_sum == 6
    assert profile.middle_hard_mismatch_tier_sum == 3
    assert profile.edge_hard_mismatch_tier_sum == 3
    assert profile.s0_class == "X"


def test_gt_wobble_is_explicit_when_allowed() -> None:
    wobble = classify_pair_profile("GGGG", "AAAC", allow_gt_wobble=True)
    strict = classify_pair_profile("GGGG", "AAAC", allow_gt_wobble=False)
