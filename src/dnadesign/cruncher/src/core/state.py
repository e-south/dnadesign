...
Core runtime primitives for state Cruncher core.
...
class SequenceState:
    ...
    def __post_init__(self) -> None:
        ...
        if arr.ndim != 1:
            raise ValueError("SequenceState.seq must be a 1-D array")
        if arr.size < 1:
            raise ValueError("SequenceState.seq must have length >= 1")
        if arr.dtype.kind not in {"i", "u"}:
            raise ValueError("SequenceState.seq must contain integer codes 0-3")
