...
--------------------------------------------------------------------------------
<cruncher project>
src/dnadesign/cruncher/src/analysis/plots/scatter_utils.py
...
def encode_sequence(seq: str, *, context: str) -> np.ndarray:
    """
    Encode an A/C/G/T sequence into int codes using _TRANS.
    Raises ValueError with context on invalid input.
    """
    if not isinstance(seq, str) or not seq:
        raise ValueError(f"{context}: sequence must be a non-empty string")
    try:
        ascii_arr = np.frombuffer(seq.encode("ascii"), dtype=np.uint8)
    except UnicodeEncodeError as exc:
        raise ValueError(f"{context}: sequence contains non-ASCII characters") from exc
    idx = _TRANS[ascii_arr]
    if np.any(idx < 0):
        invalid = sorted({chr(c) for c in ascii_arr[idx < 0]})
        raise ValueError(f"{context}: invalid base(s) {', '.join(invalid)}; expected A/C/G/T")
