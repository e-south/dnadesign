...
def summary(
    ...
    if sample is None:
        ...
        table.add_row("compute.total_sweeps", str(sample["compute"]["total_sweeps"]))
        table.add_row("compute.adapt_sweep_frac", str(sample["compute"]["adapt_sweep_frac"]))
        table.add_row("init.kind", sample["init"]["kind"])
        table.add_row("sequence_length", str(sample["sequence_length"]))
        ...
        table.add_row("elites.min_per_tf_norm", str(sample["elites"]["min_per_tf_norm"]))
        table.add_row("elites.mmr_alpha", str(sample["elites"]["mmr_alpha"]))
