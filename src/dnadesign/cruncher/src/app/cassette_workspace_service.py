...
Scaffold isolated cassette solve workspaces with deterministic runtime profiles.
...
def _write_readme(workspace_root: Path, *, solve_specs: dict[str, Path]) -> Path:
    lines = [
        ...
        "",
        "It is intentionally isolated from the general Cruncher sampling workspace flow:",
        "",
        "- cassette commands operate on the explicit `configs/cassettes/*.cassette.solve.yaml` paths here",
        (
            "- the scaffold does not auto-register with `cruncher workspaces list` "
            "because it does not include a full `configs/config.yaml`"
        ),
