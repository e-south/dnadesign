...
def test_20b_single_device_fails_on_single_45g_hopper_gpu() -> None:
    model = ModelConfig(
        id="evo2_20b",
        device="cuda:0",
        precision="bf16",
    ...
    with pytest.raises(ValidationError, match="CAPACITY_FAIL"):
        validate_model_hardware_contract(model=model, inventory=_inventory(count=1, cc="9.0"))
...
def test_20b_multi_gpu_vortex_fails_on_non_hopper_gpus_even_when_aggregate_capacity_is_sufficient() -> None:
    model = ModelConfig(
        id="evo2_20b",
        device="cuda:0",
        precision="bf16",
    ...
    with pytest.raises(ValidationError, match="requires Hopper"):
        validate_model_hardware_contract(model=model, inventory=_inventory(count=2, gib_per_gpu=45.0, cc="8.9"))
...
def test_20b_multi_gpu_vortex_passes_when_aggregate_capacity_is_sufficient() -> None:
    model = ModelConfig(
        id="evo2_20b",
        device="cuda:0",
        precision="bf16",
    ...
    validate_model_hardware_contract(model=model, inventory=_inventory(count=2, cc="9.0"))
