...
def test_candidate_inventory_uses_view_manifest_shape_without_loading_matrix(tmp_path) -> None:
    workspace_dir = _write_workspace(tmp_path)
    materialize_view(workspace_dir, "embedding_anchor")
    context = load_workspace_config(workspace_dir)
    matrix_path = context.output_root / "views" / "embedding_anchor" / "matrix.npy"
    matrix_path.write_bytes(b"candidate inventory must not inspect matrix payload bytes")
