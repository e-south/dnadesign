...
def test_phase2_alignment_distance_and_scalar_flow(tmp_path: Path) -> None:
    ...
    delta_dir = workspace_dir / "outputs" / "latentdna" / "views" / "delta20"
    ...
    scalar_table = pq.read_table(workspace_dir / "outputs" / "latentdna" / "scalars" / "delta20_norm" / "table.parquet")
    ...
    distance_table = pq.read_table(
        workspace_dir / "outputs" / "latentdna" / "distances" / "primary_landmark_distances" / "table.parquet"
    )
    ...
    expression_table = pq.read_table(
        workspace_dir / "outputs" / "latentdna" / "scalars" / "ethanol_vs_cipro" / "table.parquet"
    )
    assert "ethanol_vs_cipro" in expression_table.column_names
    assert (workspace_dir / "outputs" / "latentdna" / "alignments" / "anchor_ctx" / "manifest.json").is_file()
    assert (
        workspace_dir / "outputs" / "latentdna" / "distances" / "primary_landmark_distances" / "manifest.json"
    ).is_file()
