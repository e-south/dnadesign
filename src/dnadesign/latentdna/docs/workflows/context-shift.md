...
This workflow covers paired anchor-versus-full-context robustness metrics computed on explicit matched supports.
...
3. Compute paired geometry metrics such as `context_self_cosine`, `context_shift_l2`, and cohort-retention summaries.
4. Persist the paired sample ids used for row-level stability and cohort-retention checks.
...
- Geometry retention metrics operate on persisted paired sample ids and aligned cohort pairs, not ad hoc notebook subsets.
