...
def _apply_tf_association_overlay(
    ...
    *,
    ...
    row_key = str(association_overlay.get("row_key") or "id")
    relation_key = str(association_overlay.get("relation_key") or "usr_id")
    regulator_column = str(association_overlay.get("regulator_column") or "regulator_abbrev")
...
def _rankdata_average_local(values: np.ndarray) -> np.ndarray:
...
def _mann_whitney_u(target: np.ndarray, background: np.ndarray) -> float:
    ...
    ranks = _rankdata_average_local(combined)
