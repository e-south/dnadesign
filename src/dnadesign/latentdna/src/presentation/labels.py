...
_DIRECT_LABELS = {
    ...
    "rt_lnrna_dataset_overview": "RT-lnRNA dataset overview",
    "rt_lnrna_overlay_ordinal_audit": "Khan/Crawford abundance audit",
    "rt_lnrna_abundance_margin_ladder_gallery": "RT-lnRNA abundance group ladder",
    "rt_lnrna_abundance_margin_scatter_gallery": "RT-lnRNA abundance assay scatter",
    "rt_lnrna_slot_context_robustness_summary": "RT-lnRNA span/context robustness",
    "rt_lnrna_slot_geometry_scatter_gallery": "RT-lnRNA span geometry scatter",
    "rt_lnrna_source_structure_summary": "RT-lnRNA source/design structure",
    "rt_lnrna_candidate_decision_frontier": "RT-lnRNA candidate decision frontier",
    "rt_lnrna_candidate_x_scorecard": "RT-lnRNA candidate-X scorecard",
    "khan_abundance": "Khan PAGE RT-DNA abundance",
    "crawford_abundance": "Crawford Fig2 Eco1 msDNA score",
    "khan_rt-dna_abundance_prior": "Khan PAGE RT-DNA abundance",
    "khan rt-dna abundance prior": "Khan PAGE RT-DNA abundance",
    "crawford_eco1_msdna_abundance_prior": "Crawford Fig2 Eco1 msDNA score",
    "crawford eco1 msdna abundance prior": "Crawford Fig2 Eco1 msDNA score",
    ...
    "abundance_affiliation": "Abundance data",
    "construct_projection_status": "Construct projection",
    "label_readiness_status": "Label readiness",
    "source_window_case": "Source window",
    "compiler_generated_msd_lnrna_variant": "Compiler msDNA lnRNA variant",
    "crawford_eco1_lnrna_fixed_wt_rt": "Crawford Eco1 lnRNA fixed WT RT",
    "genbank_variant_catalog": "GenBank variant catalog",
    "in_silico_rt_cds_dms": "In silico RT CDS DMS",
    "khan_abundance_affiliated_rt_lnrna_reference": "Khan RT-lnRNA abundance reference",
    "source_promoted": "Promoted",
    "provenance_only": "Provenance only",
    "abundance_affiliated": "Has abundance data",
    "not_abundance_affiliated": "No abundance data",
    "reader_spop_labels_not_materialized": "SPOP labels not materialized",
    "within_window_materialized": "Within 2,000 bp window",
    "representable": "Representable",
    "lnrna_slot": "lnRNA span",
    "rt_cds_slot": "RT CDS span",
    "lnrna_rt_slot_pair": "lnRNA + RT CDS pair",
...
def _title_token(token: str) -> str:
    ...
    bp_match = re.fullmatch(r"(\d+)bp", lowered)
    if bp_match is not None:
        return f"{int(bp_match.group(1)):,} bp"
...
def _apply_common_phrase_rewrites(text: str) -> str:
    ...
    normalized = re.sub(r"\blnRNA plus RT slot[- ]pair\b", "lnRNA + RT CDS pair", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"\blnRNA RT slot pair\b", "lnRNA + RT CDS pair", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"\bRT CDS[- ]slot\b", "RT CDS span", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"\blnRNA[- ]slot\b", "lnRNA span", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"\bRT[- ]slot\b", "RT span", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"\bslot[- ]pair\b", "span pair", normalized, flags=re.IGNORECASE)
    return normalized
