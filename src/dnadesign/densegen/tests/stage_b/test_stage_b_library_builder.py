...
def test_build_library_for_plan_rejects_library_size_below_min_total_sites() -> None:
    plan_item = ResolvedPlanItem(
        name="demo",
        quota=1,
        include_inputs=["demo"],
        fixed_elements=FixedElements(),
        regulator_constraints=RegulatorConstraints(groups=[], min_total_sites=3),
    )
    sampling_cfg = SamplingConfig(
        pool_strategy="subsample",
        library_source="build",
    ...
    with pytest.raises(ValueError, match="min_total_sites"):
        stage_b_module.build_library_for_plan(
            source_label="demo",
            plan_item=plan_item,
            pool=pool,
...
def test_library_builder_rejects_artifact_below_min_total_sites() -> None:
    plan_item = ResolvedPlanItem(
        name="demo",
        quota=10,
        include_inputs=["demo"],
    ...
    with pytest.raises(RuntimeError, match="min_total_sites"):
        builder.build_next(library_index_start=0)
