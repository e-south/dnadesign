...
def test_build_metadata_merges_promoter_components_into_parts_detail() -> None:
    meta = build_metadata(
        sol=_DummySol(),
        plan_name="demo_plan",
        tfbs_parts=["TF1:AAAA"],
        ...
        final_sequence="ATTGACAGGGGGGTATAAT",
    )
    assert "promoter_detail" not in meta
    ...
    fixed_entries = [item for item in meta["used_tfbs_detail"] if item.get("part_kind") == "fixed_element"]
    assert len(fixed_entries) == 2


def test_build_metadata_parts_detail_normalizes_tfbs_sequence_and_fixed_geometry() -> None:
    meta = build_metadata(
        sol=_DummySol(),
        plan_name="demo_plan",
        tfbs_parts=["TF1:AAAA"],
        ...
        seq_len=10,
        actual_length=14,
        pad_meta={"used": True, "bases": 4, "end": "5prime"},
        sampling_meta={
            "achieved_length": 4,
            "relaxed_cap": False,
            "final_cap": None,
        ...
        final_sequence="AAAATTGACACCCCTATAAT",
    )
    tfbs_entries = [item for item in meta["used_tfbs_detail"] if item.get("part_kind") == "tfbs"]
    assert len(tfbs_entries) == 1
    assert tfbs_entries[0]["sequence"] == "AAAA"
    fixed_entries = [item for item in meta["used_tfbs_detail"] if item.get("part_kind") == "fixed_element"]
    assert len(fixed_entries) == 2
    upstream = next(item for item in fixed_entries if item.get("role") == "upstream")
    downstream = next(item for item in fixed_entries if item.get("role") == "downstream")
    assert upstream["offset_raw"] == 1
    assert upstream["pad_left"] == 4
    assert upstream["offset"] == 5
    assert upstream["end"] == 11
    assert downstream["offset_raw"] == 13
    assert downstream["pad_left"] == 4
    assert downstream["offset"] == 17
    assert downstream["end"] == 23
