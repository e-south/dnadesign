...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/densegen/src/core/stage_a/stage_a_progress.py
...
def _format_background_progress_line(
    *,
    input_name: str,
    phase: Optional[str],
    generated: int,
    ...
    batch_index: Optional[int],
    batch_total: Optional[int],
    reject_fimo: Optional[int],
    reject_kmer: Optional[int],
    reject_gc: Optional[int],
    reject_dup: Optional[int],
    elapsed: float,
) -> str:
    parts = [f"BG {input_name}", "background"]
    if phase:
        parts.append(f"phase {phase}")
    if target > 0:
        parts.append(f"gen {generated}/{int(target)}")
    else:
        parts.append(f"gen {generated}")
    if accepted_target > 0:
        parts.append(f"accepted {accepted}/{int(accepted_target)}")
    else:
        parts.append(f"accepted {accepted}")
    if generated > 0:
        parts.append(f"accept {_format_percent(accepted / max(1, generated))}")
    if any(x is not None for x in (reject_fimo, reject_kmer, reject_gc, reject_dup)):
        parts.append(
            f"rejects {int(reject_fimo or 0)}/{int(reject_kmer or 0)}/{int(reject_gc or 0)}/{int(reject_dup or 0)}"
        )
    if batch_index is not None:
        total_label = "-" if batch_total is None else str(int(batch_total))
        parts.append(f"batch {int(batch_index)}/{total_label}")
...
def _stage_a_live_render(state: dict[str, StageAProgressState]):
    show_accept_rate = any(row.show_accept_rate for row in state.values())
    show_rejects = any(row.show_rejects for row in state.values())
    show_tier_yield = any(row.show_tier_yield for row in state.values())
    ...
    for key in sorted(state, key=lambda k: str(state[k].motif_id)):
        ...
        accept_rate_label = "-"
        if show_accept_rate and row.show_accept_rate and accepted is not None and generated > 0:
            accept_rate = min(1.0, float(accepted) / float(generated))
            accept_rate_label = _format_percent(accept_rate)
        rejects_label = "-"
        if show_rejects and row.show_rejects:
        ...
        tier_yield = _tier_yield_label(
            ...
            show_tier_yield=show_tier_yield and row.show_tier_yield,
        )
        elapsed_label = f"{max(0.0, float(elapsed)):.1f}s"
        rate = generated / elapsed if elapsed > 0 else 0.0
        ...
        if show_accept_rate:
            row_values.append(accept_rate_label if row.show_accept_rate else "-")
        if show_rejects:
            row_values.append(rejects_label if row.show_rejects else "-")
        if show_tier_yield:
            row_values.append(tier_yield if row.show_tier_yield else "-")
        row_values.extend([batch_label, elapsed_label, rate_label])
...
def _stage_a_static_console(stream: TextIO):
    import shutil

    from rich.console import Console

    width = shutil.get_terminal_size((120, 24)).columns
    return Console(file=stream, force_terminal=False, width=width)
...
class StageAProgressManager:
    ...
    def _ensure_live(self) -> bool:
        if self._live is not None or self._static:
            return True
        self._live, self._console = _stage_a_live_start(self._stream)
        if self._live is not None:
            return True
        self._console = _stage_a_static_console(self._stream)
        self._static = self._console is not None
        return self._static
    ...
    def update(
        ...
        *,
        ...
        with self._lock:
            ...
            elif self._static and self._console is not None:
...
class _PwmSamplingProgress:
    def __init__(
        ...
        *,
        ...
        self._allow_carriage = tty and self._mode == "summary"
    ...
    def update(
        ...
        *,
        ...
        if self._use_live:
            ...
            line = _format_background_progress_line(
