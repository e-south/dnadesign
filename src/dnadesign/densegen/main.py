...
from dnadesign.utils import (
    BASE_DIR,
    ConfigLoader,
    SequenceSaver,
    generate_sequence_entry,
)


def select_solver(
    preferred: str, fallback: str, library: list, test_length: int = 10
) -> tuple[str, list]:
    ...
    try:
        ...
        print(
            f"Preferred solver {preferred} failed with error: {e}. Falling back to {fallback}."
        )
        return fallback, []
...
def _process_single_source(
    ...
    if source_config["type"].lower() == "pt":
        all_sequences = [
            entry["sequence"] for entry in data_entries if "sequence" in entry
        ]
        if not all_sequences:
            raise ValueError(
                f"PT file for source {source_label} contains no sequences."
            )
        subsample_size = densegen_config.get("subsample_size", 10)
        library_for_optim = [
            seq.strip().upper()
            for seq in random.sample(
                all_sequences, min(subsample_size, len(all_sequences))
            )
        ]
    ...
    print(
        f"Feeding {len(library_for_optim)} binding sites to the solver for source {source_label}."
    )
    ...
    out_folder_name = (
        f"densebatch_{source_config['type'].lower()}_{source_label}_n{quota}"
    )
    ...
    if progress_file.exists():
        ...
        print(
            f"Resuming from checkpoint for source {source_label}: {current_total} entries processed."
        )
        try:
    ...
    selected_solver, extra_solver_options = select_solver(
        preferred_solver, "CBC", library_for_optim or []
    )
    ...
    while global_generated < quota and (
        max_sequences is None or new_generated < max_sequences
    ):
        print(
        ...
        if source_config["type"].lower() != "pt":
            sampler = TFSampler(meta_data)
            budget_overhead = densegen_config.get("subsample_over_length_budget_by", 30)
            library_for_optim, meta_tfbs_parts = (
                sampler.generate_binding_site_subsample(
                    sequence_length, budget_overhead
                )
            )
        else:
        ...
        if max_solutions_per_subsample == 1 and fp_library in forbidden_libraries:
            print(
                f"Source {source_label}: This library has already been used. Resampling a new library."
            )
            continue
        ...
        if use_diverse_solver:
            sol_generator = opt_inst.solutions_diverse(
                solver=selected_solver, solver_options=solver_options
            )
        else:
            sol_generator = opt_inst.solutions(
                solver=selected_solver, solver_options=solver_options
            )
        ...
        for sol in sol_generator:
            ...
            subsample_progress_pct = (
                (local_generated + 1) / max_solutions_per_subsample
            ) * 100
            ...
            if fingerprint in local_forbidden:
                ...
                if consecutive_duplicates >= max_consecutive_duplicates:
                    print(
                        f"Source {source_label}: Too many consecutive duplicates; moving to a new library sample."
                    )
                    break
                continue
            consecutive_duplicates = 0
            ...
            entry = generate_sequence_entry(
                final_sol, [source_label], meta_tfbs_parts, densegen_config
            )
    ...
    print(
        f"Source {source_label}: Dense array generation complete. Total sequences: {global_generated}."
    )
...
def get_sub_batches(source_config: dict, densegen_config: dict) -> list:
    ...
    if clusters and isinstance(clusters, list) and len(clusters) > 0:
        if (
            ...
            and isinstance(promoter_constraints, list)
            and len(promoter_constraints) > 1
        ):
            for constraint in promoter_constraints:
                for cluster in clusters:
                    ...
                    base_name = sub_source_config.get(
                        "name", Path(sub_source_config["path"]).stem
                    )
                    constraint_name = constraint.get("name", "constraint")
                    sub_source_config["name"] = (
                        f"{base_name}_{cluster}_{constraint_name}"
                    )
                    sub_densegen_config.setdefault("fixed_elements", {})[
                        "promoter_constraints"
                    ] = [constraint]
            ...
            promoter_constraints
            and isinstance(promoter_constraints, list)
            and len(promoter_constraints) > 0
        ):
            for cluster in clusters:
                sub_source_config = copy.deepcopy(source_config)
                sub_densegen_config = copy.deepcopy(densegen_config)
...
def process_source(
    source_config: dict, densegen_config: dict, output_base_folder: Path
):
    """
            ...
            for sub_cfg, sub_dense_cfg in sub_batches:
                ...
                if current_count < sub_dense_cfg.get("quota", 5):
                    ...
                    _process_single_source(
                        sub_cfg, sub_dense_cfg, output_base_folder, max_sequences=1
                    )


def main():
    config_path = BASE_DIR / "src" / "dnadesign" / "densegen" / "config.yaml"
    ...
    output_base_folder = Path(__file__).parent.parent / densegen_config.get(
    ...
    if densegen_config.get("round_robin", False):
        ...
        for src_cfg in input_source_configs:
            sub_batches = get_sub_batches(
                copy.deepcopy(src_cfg), copy.deepcopy(densegen_config)
            )
        ...
        for src_cfg in input_source_configs:
            process_source(
                copy.deepcopy(src_cfg),
                copy.deepcopy(densegen_config),
                output_base_folder,
            )
