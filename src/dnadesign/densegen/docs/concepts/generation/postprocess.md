...
Use `densegen.postprocess.pad` in config for mode, pad-end selection, GC target/range, and retry bounds. For exact field contracts, use **[config reference](../../reference/config.md)**. For how postprocess interacts with solve constraints, use **[generation model](model.md)**.
