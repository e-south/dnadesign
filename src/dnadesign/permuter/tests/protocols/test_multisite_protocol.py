...
def _build_default_params(records_path: Path, art_dir: Path) -> dict:
    ...
    matching the textual spec for the RT selection scope.
    """
    return {
...
def test_multisite_select_explicit_no_cluster_mode(tmp_path):
    records_path = _write_synthetic_records(tmp_path)
    df = read_parquet(records_path).drop(columns=["cluster__perm_v1"])
    df.to_parquet(records_path, index=False)

    art_dir = tmp_path / "artifacts"
    params = _build_default_params(records_path, art_dir)
    params["select"]["clusters"]["column"] = None
    params["select"]["clusters"]["picks_per_cluster"] = None
    params["select"]["clusters"]["filters"]["min_cluster_mean_z_llr"] = None
    params["select"]["clusters"]["filters"]["min_cluster_pos_epistasis_fraction"] = None
    params["select"]["heb"]["enabled"] = False
    ...
    assert picks
    sel_df = read_parquet(art_dir / "MULTISITE_SELECT.parquet")
    assert set(sel_df["cluster_id"]) == {"all"}


def test_multisite_select_no_cluster_mode_rejects_cluster_caps(tmp_path):
    records_path = _write_synthetic_records(tmp_path)
    art_dir = tmp_path / "artifacts"
    params = _build_default_params(records_path, art_dir)
    params["select"]["clusters"]["column"] = None
    ...
    proto = MSel()
    with pytest.raises(ValueError, match="picks_per_cluster must also be null"):
        proto.validate_cfg(params=params)
