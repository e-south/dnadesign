...
Core runtime primitives for ids Permuter core.
...
def variant_id(*, scope: str, ref: str, protocol: str, sequence: str, modifications: list[str]) -> str:
    payload = {
        "scope": scope,
        "ref": ref,
        "protocol": protocol,
...
def derive_seed64(*, scope: str, ref: str, protocol: str, params: dict) -> int:
    # deterministic 64-bit seed from stable knobs
    raw = json.dumps(
        {"scope": scope, "ref": ref, "protocol": protocol, "params": params},
        separators=(",", ":"),
        sort_keys=True,
    ).encode("utf-8")
