...
def plot(
    ...
    for _, row in df_all.iterrows():
        ...
        if aa_mode:
            ...
            if "aa_pos" in row and pd.notna(row["aa_pos"]) and "aa_alt" in row and pd.notna(row["aa_alt"]):
                try:
                    records.append(
                        {"position": int(row["aa_pos"]), "to_res": str(row["aa_alt"]).upper(), "y": yv}
                    )
                except Exception:
                    pass
            # Also parse tokens
            ...
            if "nt_pos" in row and pd.notna(row["nt_pos"]) and "nt_alt" in row and pd.notna(row["nt_alt"]):
                try:
                    records.append(
                        {"position": int(row["nt_pos"]), "to_res": str(row["nt_alt"]).upper(), "y": yv}
                    )
                except Exception:
                    pass
            # Parse tokens
    ...
    ax_top.scatter(
        df1["position"], df1["_y"],
        color="lightgray", alpha=0.3, s=10, edgecolors="none", zorder=1,
    )
    ax_top.errorbar(
        stats["position"], stats["mean"], yerr=stats["sd"].fillna(0),
        fmt="o-", color="gray", alpha=0.75, markersize=3.8, capsize=2,
        elinewidth=0.8, markeredgecolor="none", zorder=2,
    )

    ref_name = df1["ref_name"].iloc[0] if "ref_name" in df1.columns and not df1.empty else ""
    ...
    for i, ch in enumerate(ref_strip, start=1):
        ax_mid.text(i, 0.5, ch, ha="center", va="center", fontsize=fs_char, family="monospace")

    # ---------- BOTTOM: heatmap ----------
    extent = (0.5, ncols + 0.5, -0.5, nrows - 0.5)
    im = ax_bot.imshow(mat, extent=extent, origin="lower", interpolation="nearest", aspect="equal")

    xticks = full_positions if ncols <= 24 else list(np.linspace(1, ncols, num=12, dtype=int))
    ...
    ax_bot.set_xlabel("Protein position" if aa_mode else "Sequence position", fontsize=11)
    ax_bot.set_ylabel("Mutated amino acid" if aa_mode else "Mutated residue", fontsize=11)
