...
Tool-scoped notify event handlers for status mapping, message rendering, and
run-state decisions.
...
class ToolEventState:
    densegen_started_at: dict[str, float] = field(default_factory=dict)
    densegen_running_state: dict[str, dict[str, Any]] = field(default_factory=dict)
...
def _densegen_flush_failed_message(event: dict[str, Any]) -> str:
    dataset_raw = event.get("dataset")
    dataset = dataset_raw if isinstance(dataset_raw, dict) else {}
    dataset_name = str(dataset.get("name") or "unknown-dataset")
    metrics_raw = event.get("metrics")
...
def _evaluate_densegen_health_event(event: dict[str, Any], run_id: str, state: ToolEventState) -> ToolEventDecision:
    ...
    if densegen_status in {"started", "start"} and event_ts_seconds is not None:
        state.densegen_started_at[str(run_id)] = event_ts_seconds
    elif densegen_status in {"resumed", "resume"} and event_ts_seconds is not None:
        state.densegen_started_at.setdefault(str(run_id), event_ts_seconds)
...
_STATUS_OVERRIDES: dict[str, Callable[[dict[str, Any]], str | None]] = {
    "densegen_health": _densegen_health_status_override,
}

_MESSAGE_OVERRIDES: dict[str, Callable[..., str]] = {
    "densegen_health": _densegen_health_message,
    "densegen_flush_failed": _densegen_flush_failed_message,
}

_EVALUATORS: dict[str, Callable[[dict[str, Any], str, ToolEventState], ToolEventDecision]] = {
    "densegen_health": _evaluate_densegen_health_event,
}


def tool_event_status_override(action: str, event: dict[str, Any]) -> str | None:
    override = _STATUS_OVERRIDES.get(action)
    if override is None:
        return None
...
def tool_event_message_override(
    ...
    *,
    ...
    renderer = _MESSAGE_OVERRIDES.get(action)
    if renderer is None:
        return None
    if renderer is _densegen_health_message:
...
def evaluate_tool_event(action: str, event: dict[str, Any], *, run_id: str, state: ToolEventState) -> ToolEventDecision:
    evaluator = _EVALUATORS.get(action)
