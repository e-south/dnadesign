...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/notify/tests/cli/test_module_layout.py
...
def _notify_root() -> Path:
    ...
    return next(
        ...
        if (parent / "core" / "contracts.py").exists() and (parent / "cli").is_dir()
    )
...
def test_notify_top_level_cli_modules_removed() -> None:
    notify_root = _notify_root()
    assert not list(notify_root.glob("cli_*.py"))
