...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/notify/tests/package/test_layout.py
...
def _notify_root() -> Path:
    ...
    return next(
        ...
        if (parent / "core" / "contracts.py").exists() and (parent / "delivery").is_dir()
    )


def test_notify_non_cli_packages_importable() -> None:
    assert importlib.import_module("dnadesign.notify.core")
    assert importlib.import_module("dnadesign.notify.core.contracts")
    assert importlib.import_module("dnadesign.notify.core.errors")
    ...
    assert importlib.import_module("dnadesign.notify.tool_events.infer")
...
def test_notify_monolith_files_removed() -> None:
    ...
    top_level_modules = {path.name for path in notify_root.glob("*.py")}
    assert top_level_modules == {"__init__.py"}
    assert not (notify_root / "contracts.py").exists()
    assert not (notify_root / "errors.py").exists()
...
def test_notify_legacy_top_level_module_paths_are_not_importable() -> None:
    legacy_modules = [
        ...
        "dnadesign.notify.contracts",
        "dnadesign.notify.errors",
