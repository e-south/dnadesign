...
_TOOL_DEFAULT_POLICY = {
    "densegen": "densegen",
    "infer_evo2": "infer_evo2",
}
...
def resolve_tool_events_path(*, tool: str, config: Path) -> tuple[Path, str | None]:
    ...
    if tool_name == "densegen":
        return _resolve_densegen_events_from_config(config), _TOOL_DEFAULT_POLICY.get(tool_name)
    if tool_name == "infer_evo2":
        return _resolve_infer_evo2_events_from_config(config), _TOOL_DEFAULT_POLICY.get(tool_name)
    allowed = ", ".join(sorted(_TOOL_DEFAULT_POLICY))
