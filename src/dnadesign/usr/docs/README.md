...
- [Keep one stress_ethanol_cipro_growth status record for current-status checks](../../../../docs/studies/stress_ethanol_cipro_growth/status-contract.md): required manifest, affiliated-dataset registry, checked-in `docs/studies/` location, status note template, refresh commands for one real study, and the `routes.md` owner handoff after the snapshot answer.
...
- [Stress ethanol/cipro status contract](../../../../docs/studies/stress_ethanol_cipro_growth/status-contract.md): maintain a study-specific manifest, affiliated-dataset registry, and status note under `docs/studies/` for current-status checks.
