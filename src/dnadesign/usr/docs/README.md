...
- [Keep one stress_ethanol_cipro_growth status record for current-status checks](../../../../docs/studies/stress_ethanol_cipro_growth/operations/catalog/status.md): required manifest, affiliated-dataset registry, checked-in `docs/studies/` location, status note template, refresh commands for one real study, and the `routes/README.md` owner handoff after the snapshot answer.
...
- [Workflow map](operations/routes/workflow-map.md): choose a command chain by intent before diving into details.
- [Sync over SSH](operations/sync/README.md): choose quickstart, setup, modes, and troubleshooting for SSH-based dataset sync.
...
- [HPC sync flow](operations/sync/hpc-agent-flow.md): preflight/run/verify loop for batch-driven workspace updates.
...
- [Stress ethanol/cipro status contract](../../../../docs/studies/stress_ethanol_cipro_growth/operations/catalog/status.md): maintain a study-specific manifest, affiliated-dataset registry, and status note under `docs/studies/` for current-status checks.
