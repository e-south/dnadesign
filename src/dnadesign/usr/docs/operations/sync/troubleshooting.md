...
7. If doctor reports `Permission denied (keyboard-interactive,hostbased)`, switch the remote to `--no-batch-mode` / `batch_mode: false`, then establish `ssh scc1` once in a terminal so the SSH ControlMaster socket is available before retrying sync commands.
...
- `Permission denied (keyboard-interactive,hostbased)`: SCC accepted publickey auth but still requires Duo or other keyboard-interactive follow-up; disable strict batch mode and pre-establish an SSH ControlMaster session.
- `Dataset directory path is outside --root and no registry.yaml ancestor was found`: pass correct `--root` or use dataset id.

## Related runbooks

- Baseline loop: [quickstart.md](quickstart.md)
- Setup and remote wiring: [setup.md](setup.md)
- Target modes and mapping: [modes.md](modes.md)
- Machine-readable decisions: [audit-loop.md](audit-loop.md)
- HPC loop: [hpc-agent-flow.md](hpc-agent-flow.md)
- Chained DenseGen/Infer flow: [chained-densegen-infer-runbook.md](chained-densegen-infer-runbook.md)
- Adversarial drills: [fidelity-drills.md](fidelity-drills.md)
