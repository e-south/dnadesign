...
- Code (CLI + library): `src/dnadesign/usr/src/`
  ...
  - Helper families should live under explicit subpackages: `cli/commands/`, `cli/support/`, `contracts/`, `datasets/`, `events/`, `genbank/`, `legacy/`, `overlays/support/`, `overlays/`, `regulondb/`, `registry/`, `runtime/`, `sequence_views/`, `storage/`, and `sync/remote/`.
  ...
  - `regulondb/` owns RegulonDB-specific overlay enrichment helpers such as source-backed functional annotation projection; keep source-specific enrichment logic out of generic dataset, registry, and sequence-view packages.
