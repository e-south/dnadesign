...
from ...datasets.mock import MockSpec
...
def cmd_make_mock(args, *, deps: ToolingDeps) -> None:
    ...
    command = (
        f"usr make-mock {args.dataset} --n {spec.n} {source} --namespace {spec.namespace} "
...
def cmd_add_demo(args, *, deps: ToolingDeps) -> None:
    ...
    command = f"usr add-demo-cols {args.dataset} --x-dim {args.x_dim} --y-dim {args.y_dim} --namespace {args.namespace}"
