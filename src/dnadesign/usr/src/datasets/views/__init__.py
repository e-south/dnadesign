...
from .reporting import describe_dataset, manifest_dataset, manifest_dict_dataset
    ...
    *,
    ...
    if fmt_norm == "csv":
        ...
        try:
            ...
            if writer is None:
                empty_columns = columns if columns else dataset.schema().names
                schema = pa.schema([field for field in dataset.schema() if field.name in empty_columns])
                pq.write_table(pa.table({}, schema=schema), str(out_path), compression=PARQUET_COMPRESSION)
        finally:
            if writer is not None:
                writer.close()


def _schema_preview(dataset: Dataset) -> list[str]:
    schema = dataset.schema()
    names = list(schema.names)
    if not names:
        raise SchemaError("Dataset schema is empty.")
    return names
