...
from ....storage.parquet import read_parquet_head
from ...support.presentation.pretty import PrettyOpts, fmt_value
from ...support.presentation.rendering import (
    print_df_plain,
    render_table_rich,
)
from . import parquet_targets
...
def _resolve_parquet_from_dir(dir_path: Path, glob: str | None = None) -> Path:
    return parquet_targets._resolve_parquet_from_dir(dir_path, glob=glob)


def _resolve_parquet_target(path_like: Path, glob: str | None = None) -> Path:
    return parquet_targets._resolve_parquet_target(path_like, glob=glob)
...
def _select_parquet_target_interactive(
    ...
    *,
    ...
    return parquet_targets._select_parquet_target_interactive(
        path_like,
        glob,
        use_rich,
        deps=deps,
...
def cmd_head(args, *, deps: ReadViewDeps) -> None:
    ...
    if deps.is_explicit_path_target(target):
        ...
        tbl = read_parquet_head(pq_path, n, columns=cols)
        df = tbl.to_pandas()
    ...
    df = d.head(n, columns=cols, include_deleted=bool(getattr(args, "include_deleted", False)))
    df = _reorder_head_columns_for_display(df, explicit_columns=cols)
