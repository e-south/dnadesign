...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/usr/src/cli/commands/deps.py
...
from . import namespace as namespace_commands
from . import query as query_commands
from . import read_views as read_views_commands
from . import tooling as tooling_commands
from .lifecycle import materialize as lifecycle_materialize_commands
from .lifecycle import snapshot as lifecycle_snapshot_commands
...
def build_runtime_deps(
    *,
    ...
    return query_commands.RuntimeDeps(
...
) -> lifecycle_materialize_commands.MaterializeDeps:
    return lifecycle_materialize_commands.MaterializeDeps(
        resolve_dataset_name_interactive=resolve_dataset_name_interactive,
...
def build_snapshot_deps(
    *,
    resolve_dataset_name_interactive: Callable[[Path, str | None, bool], str | None],
) -> lifecycle_snapshot_commands.SnapshotDeps:
    return lifecycle_commands.SnapshotDeps(
        resolve_dataset_name_interactive=resolve_dataset_name_interactive,
    )
...
def build_merge_deps(
    *,
    ...
    return maintenance_commands.MergeDeps(
...
def build_namespace_deps(
    *,
    ...
    return namespace_commands.NamespaceDeps(
