...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/usr/src/cli/support/resolution/paths.py
...
from ....dataset import ARCHIVE_DATASET_PREFIX, RECORDS, Dataset
from ....datasets.core.identity import ARCHIVED_DATASET_ID_ERROR
from .roots import normalize_usr_root, pkg_usr_root

LEGACY_DATASET_PATH_ERROR = ARCHIVED_DATASET_ID_ERROR
HISTORICAL_ARTIFACT_PATH_ERROR = (
    "Paths under retired 'src/dnadesign/usr/archived/' are historical artifacts, not dataset roots. "
    "Use datasets/archived/<dataset> for archived USR datasets or "
    "datasets/archived/promoter_misc_pytorch/ for legacy PyTorch artifacts."
)
...
def assert_not_legacy_dataset_path(
    ...
    *,
    ...
    historical_root = (pkg_root / "archived").resolve()
    if target == historical_root or target.is_relative_to(historical_root):
        raise SequencesError(HISTORICAL_ARTIFACT_PATH_ERROR)
...
def assert_supported_root(root: Path, *, pkg_root: Path | None = None) -> None:
    ...
    pkg_root = Path(pkg_root).resolve() if pkg_root is not None else pkg_usr_root().resolve()
    historical_root = (pkg_root / "archived").resolve()
    if root_resolved == historical_root:
        raise SequencesError(
            f"Historical USR artifact root is not supported: {root_resolved}. {HISTORICAL_ARTIFACT_PATH_ERROR}"
        )
...
def resolve_dataset_for_read(
    ...
    *,
    ...
    if dataset_name == ARCHIVE_DATASET_PREFIX or dataset_name.startswith(f"{ARCHIVE_DATASET_PREFIX}/"):
