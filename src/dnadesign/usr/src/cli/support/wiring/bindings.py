...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/usr/src/cli/support/wiring/bindings.py
...
def build_cli_bindings(
    *,
    ...
    def cmd_init(args):
        lifecycle_commands.cmd_init(args)

    def cmd_import(args):
        lifecycle_commands.cmd_import(args, resolve_path_anywhere=resolve_path_anywhere)

    def cmd_attach(args):
        lifecycle_commands.cmd_attach(args, resolve_path_anywhere=resolve_path_anywhere)

    def cmd_info(args):
        query_commands.cmd_info(
            args,
            resolve_output_format=resolve_output_format,
            print_json=print_json,
            output_version=output_version,
    ...
    def cmd_validate(args):
        query_commands.cmd_validate(args, deps=runtime_deps())
    ...
    def cmd_event_log_garden(args):
        maintenance_commands.cmd_event_log_garden(args, deps=maintenance_deps())

    def cmd_events_tail(args):
        query_commands.cmd_events_tail(args, deps=runtime_deps())

    def cmd_get(args):
        query_commands.cmd_get(args, deps=runtime_deps())

    def cmd_grep(args):
        query_commands.cmd_grep(args, deps=runtime_deps())

    def cmd_export(args):
        query_commands.cmd_export(args, deps=runtime_deps())

    def cmd_delete(args):
        lifecycle_commands.cmd_delete(args)

    def cmd_restore(args):
        lifecycle_commands.cmd_restore(args)

    def cmd_state_set(args):
        lifecycle_commands.cmd_state_set(args)

    def cmd_state_clear(args):
        lifecycle_commands.cmd_state_clear(args)

    def cmd_state_get(args):
        lifecycle_commands.cmd_state_get(
            args,
            resolve_output_format=resolve_output_format,
            print_json=print_json,
            output_version=output_version,
    ...
    def cmd_materialize(args):
        lifecycle_commands.cmd_materialize(args, deps=materialize_deps())

    def cmd_snapshot(args):
        lifecycle_commands.cmd_snapshot(args, deps=snapshot_deps())
    ...
    def cmd_merge_datasets(args):
        maintenance_commands.cmd_merge_datasets(args, deps=merge_deps())
    ...
    def cmd_namespace_list(args):
        namespace_commands.cmd_namespace_list(args, deps=namespace_deps())

    def cmd_namespace_show(args):
        namespace_commands.cmd_namespace_show(args, deps=namespace_deps())

    def cmd_namespace_register(args):
        namespace_commands.cmd_namespace_register(args, deps=namespace_deps())
