...
def update_meta_part_type(file_path: str) -> None:
    ...
    if parent_dir.startswith("densebatch_"):
        ...
        user_input = (
            input(f"Directory '{parent_dir}' is a densebatch directory. Define dense arrays by constraint? [y/n]: ")
            .strip()
            .lower()
        )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Validate and inspect legacy archived PyTorch batch files under the USR archive bucket."
    )
    parser.add_argument(
        "--path",
        type=str,
        ...
        help=(
            "File or directory path. Defaults to the archived promoter-focused PyTorch bucket. "
            "If a file is given, --index (and optionally --key) must be provided."
        ),
    )
    parser.add_argument(
        "--index",
