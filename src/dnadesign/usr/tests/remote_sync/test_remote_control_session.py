...
def test_control_session_status_reports_live_socket(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("dnadesign.usr.src.remote_sync.remote.subprocess.run", _run)
...
def test_warm_auth_requires_tty_when_socket_not_live(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("dnadesign.usr.src.remote_sync.remote.subprocess.run", _run)
    monkeypatch.setattr("dnadesign.usr.src.remote_sync.remote.sys.stdin", _FakeTTY(is_tty=False))
    monkeypatch.setattr("dnadesign.usr.src.remote_sync.remote.sys.stdout", _FakeTTY(is_tty=False))
...
def test_warm_auth_bootstraps_missing_socket(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("dnadesign.usr.src.remote_sync.remote.subprocess.run", _run)
    monkeypatch.setattr("dnadesign.usr.src.remote_sync.remote.sys.stdin", _FakeTTY(is_tty=True))
    monkeypatch.setattr("dnadesign.usr.src.remote_sync.remote.sys.stdout", _FakeTTY(is_tty=True))
