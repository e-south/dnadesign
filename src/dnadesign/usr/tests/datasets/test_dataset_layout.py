...
def test_normalize_dataset_id_rejects_legacy_archived_prefix() -> None:
    with pytest.raises(SequencesError, match="reserved for archived storage"):
...
def test_dataset_open_accepts_explicit_archived_dataset_path(tmp_path: Path) -> None:
    archive_root = tmp_path / "archived"
    _init_dataset(archive_root, "demo")

    dataset = Dataset.open(tmp_path, str(archive_root / "demo"))

    assert dataset.root == archive_root
