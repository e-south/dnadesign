...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/usr/tests/cli/commands/query/test_cli_format_json.py
...
def test_cli_info_json_accepts_dataset_path_outside_root(tmp_path: Path) -> None:
    ...
    ds = Dataset(dataset_root, "densegen_demo")
    ds.init(source="test")
    ds.import_rows(
        [
            {"sequence": "ACGT", "bio_type": "dna", "alphabet": "dna_4", "source": "unit"},
    ...
    payload = json.loads(result.stdout)
    assert payload["usr_output_version"] == 1
    assert payload["data"]["name"] == "densegen_demo"
    assert payload["data"]["path"].endswith("densegen_demo/records.parquet")
