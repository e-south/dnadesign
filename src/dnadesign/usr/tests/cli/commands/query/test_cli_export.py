...
--------------------------------------------------------------------------------
dnadesign
src/dnadesign/usr/tests/cli/commands/query/test_cli_export.py
...
def test_cli_export_parquet_file_target(tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        app,
        ["--root", str(root), "export", "densegen_demo", "--fmt", "parquet", "--out", str(out)],
    )
...
def test_cli_export_directory_target_uses_dataset_filename(tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        app,
        ["--root", str(root), "export", "densegen_demo", "--fmt", "parquet", "--out", str(out_dir)],
    )
