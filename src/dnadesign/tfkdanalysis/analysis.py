...
def generate_volcano_plot(df: pd.DataFrame, fc_column: str, logp_column: str, config: dict, output_path: Path) -> pd.DataFrame:
    sns.set(style="ticks")
    try:
    ...
    if sizes is not None:
        plt.scatter(groupA[fc_column], groupA[logp_column],
                    s=sizes[groupA.index],
                    color="lightgray", alpha=0.35, marker="o", edgecolor="none")
        plt.scatter(groupB[fc_column], groupB[logp_column],
                    s=sizes[groupB.index],
                    color="gray", alpha=0.35, marker="o", edgecolor="none")
    else:
        plt.scatter(groupA[fc_column], groupA[logp_column],
                    color="lightgray", alpha=0.35, marker="o", edgecolor="none")
        plt.scatter(groupB[fc_column], groupB[logp_column],
        ...
        for i, reg in enumerate(regulators):
            ...
            if sizes is not None:
                if not reg_non_sig.empty and reg_sig.empty:
                    plt.scatter(reg_non_sig[fc_column], reg_non_sig[logp_column],
                                s=sizes[reg_non_sig.index],
                                color=light_color, alpha=0.65, marker=marker_style, label=reg, edgecolor="none")
                else:
                    plt.scatter(reg_non_sig[fc_column], reg_non_sig[logp_column],
                                s=sizes[reg_non_sig.index],
                                color=light_color, alpha=0.65, marker=marker_style, edgecolor="none")
                    plt.scatter(reg_sig[fc_column], reg_sig[logp_column],
                                s=sizes[reg_sig.index],
                                color=palette[i], alpha=0.65, marker=marker_style, label=reg, edgecolor="none")
            else:
                if not reg_non_sig.empty and reg_sig.empty:
                    plt.scatter(reg_non_sig[fc_column], reg_non_sig[logp_column],
                                color=light_color, alpha=0.65, marker=marker_style, label=reg, edgecolor="none")
            ...
            if config.get("annotate_operon", True):
                for _, row in reg_sig.iterrows():
                    plt.text(row[fc_column] + x_offset, row[logp_column],
                             str(row.get("operon", "")),
    ...
    plt.title(f"Promoter activity changes by TF Knockdown (M9-glucose)", fontsize=12)
...
def export_regulator_csvs(df: pd.DataFrame, regulators: list, fc_column: str, threshold: float, output_dir: Path) -> None:
    """
...
def generate_regulator_scatter_plot(df: pd.DataFrame, regulators: list, fc_column: str,
                                    global_xlim: tuple, config: dict, output_path: Path) -> None:
    """
        ...
        if not nonsig_df.empty and sig_df.empty:
            ...
            if config.get("annotate_operon", True):
                for _, point in sig_df.iterrows():
                    if point["operon"]:
                        plt.text(point["fc"] + 0.02, point["y"], str(point["operon"]),
                                fontsize=8, ha="left", va="bottom")
