...
--------------------------------------------------------------------------------
<dnadesign project>
seqfetcher/regulondb_13_promoter_RpoH_set.py

Ingests the RegulonDB 13 Promoter RpoH set from:
    DATA_FILES["regulondb_13_promoter_RpoH_set"]
...
def ingest():
    df = load_dataset("regulondb_13_promoter_RpoH_set", header=0)
    sequences = []
    for idx, row in df.iterrows():
        ...
        entry = {
            ...
            "name": name,
            "sequence": seq,
            "meta_source": "regulondb_13_promoter_RpoH_set",
...
def save_output(sequences):
    output_dir = Path(BASE_DIR) / "sequences" / "seqbatch_regulondb_13_promoter_RpoH_set"
    output_dir.mkdir(parents=True, exist_ok=True)
    saver = SequenceSaver(str(output_dir))
    saver.save(sequences, "seqset_regulondb_13_promoter_RpoH_set.pt")
    summary = {
        "date_created": datetime.datetime.now().isoformat(),
        "source_file": str(DATA_FILES["regulondb_13_promoter_RpoH_set"]),
