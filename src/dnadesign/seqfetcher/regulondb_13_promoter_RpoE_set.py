...
def save_output(sequences):
    output_dir = Path(BASE_DIR) / "src" / "dnadesign" / "sequences" / "seqbatch_regulondb_13_promoter_RpoE_set"
    output_dir.mkdir(parents=True, exist_ok=True)
    saver = SequenceSaver(str(output_dir))
    additional_info = {"source_file": "regulondb_13_promoter_RpoE_set", "part_type": "promoter"}
    saver.save_with_summary(sequences, "seqbatch_regulondb_13_promoter_RpoE_set.pt", additional_info=additional_info)
