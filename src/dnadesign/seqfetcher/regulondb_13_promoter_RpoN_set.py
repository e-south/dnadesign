...
--------------------------------------------------------------------------------
<dnadesign project>
seqfetcher/regulondb_13_promoter_RpoN_set.py

Ingests the RegulonDB 13 Promoter RpoN set from:
    DATA_FILES["regulondb_13_promoter_RpoN_set"]
...
def ingest():
    df = load_dataset("regulondb_13_promoter_RpoN_set", header=0)
    sequences = []
...
def save_output(sequences):
    output_dir = Path(BASE_DIR) / "src" / "dnadesign" / "sequences" / "seqbatch_regulondb_13_promoter_RpoN_set"
    output_dir.mkdir(parents=True, exist_ok=True)
    saver = SequenceSaver(str(output_dir))
    additional_info = {"source_file": "regulondb_13_promoter_RpoN_set", "part_type": "promoter"}
    saver.save_with_summary(sequences, "seqbatch_regulondb_13_promoter_RpoN_set.pt", additional_info=additional_info)
