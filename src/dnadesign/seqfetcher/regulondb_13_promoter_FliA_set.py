...
def save_output(sequences):
    output_dir = Path(BASE_DIR) / "sequences" / "seqbatch_regulondb_13_promoter_FliA_set"
    output_dir.mkdir(parents=True, exist_ok=True)
    saver = SequenceSaver(str(output_dir))
    saver.save(sequences, "seqset_regulondb_13_promoter_FliA_set.pt")
    summary = {
        "date_created": datetime.datetime.now().isoformat(),
        "source_file": str(DATA_FILES["regulondb_13_promoter_FliA_set"]),
